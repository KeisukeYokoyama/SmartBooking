...
## 🧪 テスト負債の解消: フォーム幅テストの陳腐化（2026-09-12・**テストのみ／リリース無し**・commit `a385aef`）
...
**差分は `tests/` と `docs/` と `.gitignore` のみ＝配布物・バージョン・WordPress.org は無関係。**

### 何が起きていたか

公開中のヘルプ画像が氏名欄を **`お名前`** と表示していた（実装の初期値は **`氏名`**）。
原因は**撮影環境の DB を回帰スイートと共有しているのに、撮影シードが初期3項目をそろえ直していなかった**こと。
`tests/e2e/phase2-helpers.js` の `restoreSnapshot()` が `field_label='お名前'` を UPDATE し、
既定フォームの初期3項目は activator が作る行＝シードのレジストリ外なので**上書きされないまま撮られていた**。
画像セット内部の不統一（`conditional-fields/02` だけ `氏名`）も同じ原因の裏返し
（02 はデモフォームを使うカットで、デモ側の定義は元から `氏名`）。

### 是正（シードが出荷コードの初期値へそろえ直す）

`normalize_protected_fields()` を追加。**値をシード側へ複製しない**のが要点で、
`Smart_Booking_Activator::seed_initial_fields_for_form()` を**実在しないフォーム id**
（`PROBE_FORM_ID = 999999999`）に対して 1 度走らせ、生成された 3 行を読んで**必ず削除**し、
読み取った値を既定フォームへ UPDATE する。コピーすると出荷コードが変わったとき黙って食い違う——
それが本件そのものだったため。`forms` テーブルには触らないのでショートコードの `form_id` は動かない。

- 変更前の値はレジストリ `protected_fields` 区画へ**初回だけ**退避し、**purge で 1 列ずつ復元**。
  実測: purge 後に `お名前` / placeholder 空 / sort_order 10,20,30 へ完全復帰。レジストリ option も削除された。
- 2 回目以降のシードは `変更なし` と出る（冪等・実測）。レジストリ構造バージョン 2 → 3。
- デモフォーム側（`field_defs()`）は自動で書き換えず、出荷コードとずれたら**サマリに警告**
  （`protected_field_drift()`）。今回は警告ゼロ＝一致を実測確認。

### 全件確認で新たに見つかったずれ（ラベル以外に 3 件）

| 列 | 撮影環境 | 出荷コードの初期値 | 画像への影響 |
|---|---|---|---|
| `customer_name.field_label` | `お名前` | **`氏名`** | あり（本件） |
| `customer_name.placeholder` | **空** | **`山田 太郎`** | あり（フロントに薄字が出ない） |
| `customer_name.field_options` | `[]` | 空文字 | 無し（text 型では読まれない） |
| 初期3項目の `sort_order` | 10/20/30 | 0/1/2 | 無し（相対順序が同じ） |

店舗名・担当者名・フォーム名（`デフォルト` / `標準フォーム`）は実装と一致しており、ずれは無かった。

### 影響カットの機械判定（新しい道具を入れた）

`tests/screenshots/helpers.js` の `shot()` が、撮影と同時に
`screenshot-text/<slug>/<name>.txt` へ **innerText ＋ placeholder / value / 選択中 option** を書き出す。
以後 `grep -rl 'お名前' screenshot-text/` で候補が出る。

判定は **修正前の DB 状態で 1 回・修正後で 1 回撮り、ダンプを 1 対 1 で diff** した。

- **差分が出たカット = 9 枚**（`address-field/01,02` / `conditional-fields/01,03,04,05,06` / `custom-fields/01,02`）。
- **そのうち画像に実際に写っていたのは 6 枚**（`address-field/01` / `conditional-fields/01,05,06` / `custom-fields/01,02`）。
  残り 3 枚はフロントのカットで、氏名欄がビューポート外だった（`fullPage:false`）。
- ⚠️ **ダンプは「DOM に出ていた文字」であって「画像に写っている文字」ではない。**
  機械判定で候補を絞り、最後は目視。
- ⚠️ **テキストダンプを `test-results/` 配下に置かないこと。** Playwright は実行のたびに
  `outputDir` を丸ごと削除するため、`--grep` で 1 カット撮り直すと他のダンプが消える（実際に踏んだ）。
  `screenshot-text/`（.gitignore 済み）に出している。

### 撮り直し（実施済み・サイト側へのコピーは未実施）

**全 29 枚を 1 セッションで撮り直した**（予約 id・相対日付・バージョンバッジを世代でそろえるため）。
`stores/01-store-list.png` / `stores/02-store-add-modal.png` も含む（サイト側が旧画像のため）。
撮影後 purge 済み。`grep -rl 'お名前' screenshot-text/` は **design の 2 枚のみ**（下記）。

### ⚠️ 出荷コードのため直していない件（停止して報告）

- **`src/admin/pages/formsettings/FormPreview.jsx:58` が `お名前` をハードコード**している。
  デザインタブのフォームプレビューは静的モックで、**同一画面内にフィールド一覧の `氏名` と並ぶ**。
  `design/01-design-tab.png` / `design/02-design-availability-colors.png` の 2 枚には今も `お名前` が写る。
  起票 = `docs/bugs/form-preview-label-mismatch.md`（案A: 1 行で `氏名` に直す）。**次のリリースに同梱するのが妥当。**
- 関連: `includes/rest/class-rest-public.php:953` のエラー文も `お名前を入力してください。`。
  ラベルはユーザーが変更できるので**別途の判断**が要る（起票に記載）。

### ⚠️ 回帰フィクスチャが画像に写る件（判断済み・今回は直さない）

`店舗1` / `担当者1` が `stores/01` `stores/02` に**実際に写っている**（バッジも `店舗 4` / `担当者 5`）。
起票 = `docs/bugs/screenshot-env-shares-regression-fixtures.md`。

**判断: シードにフィクスチャ行を消させる方式は採らない。**
このシードは「レジストリ外の行は読むだけ」を明文の契約にしており、**他の持ち主の行を消すのは
ユーザーの実データが入った環境で破壊的**になる。名前で狙い撃ちする実装も wp-env 固有のハックで脆い。
**本来の解は「回帰フィクスチャの無い DB で撮影セッションを走らせる」**（撮影専用 wp-env、
または撮影前の DB リセット手順）。環境レベルの変更なので別タスク。

> なお **シードのコードは tests/e2e と共有していない**（`tests/screenshots/seed/` は完全に独立）。
> 共有しているのは **wp-env の DB** だけ。今回の是正は「共有状態に期待せず、撮影側が自分で宣言し直して
> purge で戻す」形にしたもので、フィールドについては実質的な独立を達成している。
