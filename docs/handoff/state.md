...
## 🔴 現在の公開状況（最優先・2026-08-16 更新）

> ⚠️ 本ファイル下部の「現在地」節は 2026-07-30 時点の記述で **stale**（「公開版 v0.5.0／v0.5.1 未公開」は**誤り**）。**実体を優先すること**（下記「バージョン状態の確認手順」で必ず実機確認してから作業する）。

- **WordPress.org 公開版 = v0.5.1**（SVN **rev 3627795**・**2026-07-30 公開済み**、コミット者 liberdadeinc）。trunk == tags/0.5.1 == v0.5.1（`diff -rq` 差分ゼロ・混入なし）。
- **main = v0.5.1**（HEAD `05dfe4d`・バージョン4箇所一致）。**タグ `v0.5.1` は push 済み**（`origin` に `refs/tags/v0.5.1` = `05dfe4d…` 確認）。`origin/main..main` 空＝**main も push 済み**。
- **⚠️ 未コミットの開発成果あり: v0.5.2 sort_order 修正 → `git stash@{0}`（"v0.5.2 sort_order fix (WIP)"）に退避中**。**次セッションはこれを誤って破棄しないこと**。`git stash pop` で復元（復元済みなら作業ツリーに7ファイル・+126/−22）。詳細は下記「v0.5.2」。
...
| **0.5.1** | **3627795** | **2026-07-30** | **空き状況表示のカスタマイズ（しきい値／文言／色）← 最新公開** |
...
### v0.5.2 sort_order 修正（実装・実機検証 完了／未コミット＝stash@{0}）
...
- **次の一手**: レビュー → v0.5.2 bump（4箇所＋readme Changelog 日本語）→ ZIP → **人間 GO で SVN 公開**（trunk 上書き→`svn cp trunk tags/0.5.2`→`svn ci`）。
