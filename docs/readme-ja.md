# readme.txt 日本語原文（GlotPress 投入用の原稿）

readme.txt を英語ソース化するにあたり、コミット `5facdc8`（v0.5.3 時点）の readme.txt に含まれていた日本語セクションをそのまま保存したもの。
translate.wordpress.org（GlotPress）へ ja 翻訳を投入する際の原稿として使う。**1文字も改変しないこと。**

対象セクション: Installation / Frequently Asked Questions / Screenshots / External services / Changelog / Upgrade Notice
（Description セクションは v0.5.3 時点で既に英語だったため対象外）
