...
- 修正: 管理画面で、変更できない状態（グレー表示）のセレクトボックスの中に、ドロップダウンの矢印が一面に繰り返し表示される不具合を修正しました。あわせて、入力エラーが表示されているセレクトボックスから矢印が消えてしまう不具合も修正しました（どちらも同じ指定が原因です）。見た目だけの問題で、保存される内容や送信されるメールには影響しません。

## 0.5.5 – 2026-09-11

- 修正: 既定以外のフォームに追加した入力項目の回答が、管理画面の予約詳細に表示されない不具合を修正しました。回答は正しく保存されており CSV にも出力されていましたが、管理画面からは確認できない状態でした。予約詳細が「その予約が実際に申し込まれたフォーム」の入力項目を読むようになります。
- 修正: 「管理者へのメール」をオフにしていて店舗のメールアドレスが未登録の場合、担当者宛の通知が何の表示もないまま送信されない状態でした。担当者にメールアドレスが登録されているのに通知が届かない場合、メール設定画面に注意が表示されるようになります。
- 修正: メール設定画面と担当者の編集画面の説明が「店舗・担当者宛の通知は引き続き送信されます」と断定していましたが、実際には店舗のメールアドレスの登録が条件です。実際の条件に合わせて説明を修正しました。
- 修正: 既存の入力項目のタイプを「住所（郵便番号）」に変更すると、住所の自動入力が「自動入力しない」側で開いていました。住所カードから新規追加した場合（自動入力する）と既定が逆だったため、変更時も「自動入力する」で開くように揃えました。
- 修正: メール変数 `{schedule_time}` の説明に書かれていた例が、実際に差し込まれる内容と食い違っていました（「14:00〜」→「14:00〜15:00」）。
- 修正: readme.txt に記載していた zipcloud 利用規約へのリンクが http だったため https に修正しました。

## 0.5.4 – 2026-09-11

- 修正: 表示条件の親に指定していたラジオボタン／セレクトボックスの項目を、あとからチェックボックスに変更できてしまい、条件付きの項目に入力された回答が無言で破棄される不具合を修正しました。予約フォームにも確認画面にも項目が表示され、予約自体は正常に完了するにもかかわらず、回答は保存されず、予約詳細・CSV・通知メールのいずれにも現れない状態でした。他の項目の表示条件に使われている項目は、ラジオボタン／セレクトボックス以外の種別に変更できなくなり、すでにその設定になっているサイトでも条件付きの項目が表示されなくなります。
- 注意: 該当する設定を使っていたサイトでは、過去の予約でその項目が空欄になっている可能性があります。失われた回答はデータベースに記録自体が残っていないため、復旧できません。
