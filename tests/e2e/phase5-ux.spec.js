...
function wpDb(sql) {
	execSync(`npx wp-env run cli wp db query "${sql.replace(/"/g, '\\"')}"`, {
...
test.describe('Phase 5 UX 修正 5 件 ピンポイント検証', () => {
	...
	// ① UX-2: few_left の境界条件
	// =========================================================================
	test('UX-2: capacity=3/booked=1 (残2) → few_left（旧ロジックでは available）', async ({ page }) => {
		const d = ymd(1);
		...
		wpDb(`UPDATE wp_smb_schedules SET booked_count = 1 WHERE id = ${sid};`);
	...
	test('UX-2: capacity=2/booked=1 (残1) → few_left（旧ロジックでは available）', async ({ page }) => {
		const d = ymd(1);
		const sid = insertSchedule({
			storeId: 1,
			staffId: 1,
	...
	test('UX-2: capacity=10/booked=7 (残3) → few_left（旧と不変）', async ({ page }) => {
		const d = ymd(1);
		const sid = insertSchedule({
			storeId: 1,
			staffId: 1,
		...
		wpDb(`UPDATE wp_smb_schedules SET booked_count = 7 WHERE id = ${sid};`);
		await gotoFrontForm(page);
		const res = await publicRest(page, 'public/availability', {
			query: { store_id: 1, staff_id: 1, date_from: d, date_to: d },
		});
	...
	test('UX-2: capacity=10/booked=6 (残4) → available（回帰確認）', async ({ page }) => {
		const d = ymd(1);
		const sid = insertSchedule({
			storeId: 1,
			staffId: 1,
		...
		wpDb(`UPDATE wp_smb_schedules SET booked_count = 6 WHERE id = ${sid};`);
		await gotoFrontForm(page);
		const res = await publicRest(page, 'public/availability', {
			query: { store_id: 1, staff_id: 1, date_from: d, date_to: d },
		});
	...
	test('UX-9 追記: 409 後 → 「日付を選び直す」 → date ステップで時間枠が「未選択状態」になる', async ({
		...
		await page.locator('#smb-front-field-customer_name').fill('リセット 太郎');
		await page.locator('#smb-front-field-customer_email').fill('reset@example.com');
		await page.locator('#smb-front-field-customer_phone').fill('090-0000-0002');
		...
		await page.waitForSelector('.smb-front-time-slots', { timeout: 10_000 });
	...
	// ⑤ UX-12: 完了画面の日時フォーマット統一 (日付 + 時間 [start 〜 end])
	// =========================================================================
	test('UX-12: 完了画面に「日付」「時間」の 2 行が表示され、旧「日時」が存在しない', async ({ page }) => {
		const d = ymd(1);
		insertSchedule({
			storeId: 1,
			staffId: 1,
		...
		await page.locator('.smb-front-day-tile:not(.is-disabled)').first().click();
		await page.getByRole('button', { name: /14:00から15:00/ }).click();
		await page.locator('#smb-front-field-customer_name').fill('完了 太郎');
		await page.locator('#smb-front-field-customer_email').fill('done@example.com');
