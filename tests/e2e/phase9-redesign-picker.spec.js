 ...
 *
 * NOTE:
 *   - phase3-helpers の restoreBaseline は smart_booking_calendar_view_mode を delete するため、
...
test.describe( 'Phase 9 Eval-3: 日付・時間ピッカー検証', () => {
	...
	test( '満席の時間枠は disabled かつ「×」プレフィックスが含まれる', async ( {
		...
		wpCli(
			`wp db query "UPDATE wp_smart_booking_schedules SET booked_count=1 WHERE schedule_date='${ dateStr }' AND start_time='10:00:00';"`
		);
		...
		try {
			...
			wpCli(
				`wp db query "DELETE FROM wp_smart_booking_schedules WHERE schedule_date='${ dateStr }';"`
			);
		}
	} );
	...
	test( 'calendar_mode=month_only: フロント初期表示で月グリッドが表示される', async ( {
		...
		// afterEach の restoreBaseline で smart_booking_calendar_view_mode は delete される.
	} );
} );
