 ...
 *   - 確認画面「修正する」で入力が保持される
 *   - 完了メッセージ（smart_booking_completion_message）の反映
 *   - スキップルール:
 *       * 店舗1・担当者1 → いきなり日付選択
 *       * 店舗1・担当者2 → 担当者選択から
...
test.describe( 'Phase 3 Eval-1: 予約フロー 正常系 + スキップルール', () => {
	...
	test( '完走フロー: 店舗2・担当者2 → 全ステップ通過して予約番号が表示される', async ( {
		...
		// 店舗選択・担当者選択ステップを表示するには show_*_front=1 が必要.
		...
		// 店舗・担当者・日時は .smb-front-confirm-summary に表示される（Gen-D 仕様）.
		...
		// 顧客入力データは .smb-front-confirm セクションに表示される.
	...
	test( 'smart_booking_completion_message を設定すると完了画面に反映される', async ( {
		page,
	} ) => {
		setOption(
	...
	test( 'スキップ: 店舗1・担当者2 → 担当者選択から（店舗スキップ）', async ( {
		...
		// 担当者選択ステップを表示するには show_staff_front=1 が必要.
		setOption( 'smart_booking_show_staff_front', '1' );
	...
	test( 'スキップ: 店舗2・各店舗の担当者1 → 店舗選択後に担当者スキップ', async ( {
		...
		// 店舗選択ステップを表示するには show_store_front=1 が必要.
		setOption( 'smart_booking_show_store_front', '1' );
	...
	test( '[smart_booking store_id="X"]: 指定店舗固定・店舗選択スキップ（page_id=7 を一時的に書き換える）', async ( {
		...
		// store_id=store2 を指定したショートコードへ予約フォームページ本文を差し替える.
		// FRONT_PAGE_PATH = '/?page_id=5' なので page_id=5 を使用する.
		const { execSync } = require( 'node:child_process' );
		const path = require( 'node:path' );
		// 実際のページIDを動的に取得する（wp-env 環境によって ID が異なる場合に対応）.
		const pageIdRaw = execSync(
		...
		// 出力から数字行だけを取り出す（CSV ヘッダー行を除外）。見つからない場合は 5 を使う.
		const numericLine = pageIdRaw.split( '\n' ).find( ( l ) => /^\d+$/.test( l.trim() ) );
		const frontPageId = numericLine ? parseInt( numericLine.trim(), 10 ) : 5;
		const replaceCmd = `npx wp-env run cli wp post update ${ frontPageId } --post_content='<!-- wp:shortcode -->[smart_booking store_id="${ store2 }"]<!-- /wp:shortcode -->'`;
		execSync( replaceCmd, {
			cwd: path.resolve( __dirname, '..', '..' ),
			encoding: 'utf8',
			stdio: [ 'ignore', 'pipe', 'pipe' ],
		...
		try {
			...
			execSync(
				`npx wp-env run cli wp post update ${ frontPageId } --post_content='<!-- wp:shortcode -->[smart_booking]<!-- /wp:shortcode -->'`,
				{
					cwd: path.resolve( __dirname, '..', '..' ),
					encoding: 'utf8',
					stdio: [ 'ignore', 'pipe', 'pipe' ],
	...
	test( '表示順序 A（デフォルト）: date → time → form の順', async ( {
		...
		seedWeekSchedules( USER_STORE_ID, USER_STAFF_ID );
		// baseline で smart_booking_booking_flow_order は未設定 = デフォルト 'A'.
		await gotoFrontForm( page );
		// 日付選択が先.
		await expect(
			page.getByRole( 'heading', { name: '日付選択' } )
	...
	test( 'ブラウザリロードで state は初期化される', async ( { page } ) => {
		...
		// Gen-D: MainInputPage は日付・フォームを 1 画面統合しているため、
		// リロード後も日付選択セクションとフォームは同時に DOM に存在する。
		// state がリセットされたことは「確認ボタンが disabled」で確認する（date/time 未選択）。
		await expect(
