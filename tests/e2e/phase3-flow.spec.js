 ...
 * @returns {{ firstSelectable: string, scheduleIds: number[] }}
 */
...
test.describe('Phase 3 Eval-1: 予約フロー 正常系 + スキップルール', () => {
	...
	test('フォームの初期描画: React エラー 0 件（baseline: store=1, staff=1, schedule=0）', async ({ page }) => {
		const errors = [];
	...
	test('公開 REST: nonce 付きで 200、nonce なしで 403', async ({ page }) => {
		await gotoFrontForm(page);
		...
		const noNonceRes = await publicRest(page, 'public/stores', { sendNonce: false });
		expect(noNonceRes.status).toBe(403);
	});

	// ---- 完走フロー（店舗2・担当者2）----

	test('完走フロー: 店舗2・担当者2 → 全ステップ通過して予約番号が表示される', async ({ page }) => {
		// 追加店舗と担当者を追加.
		const store2 = insertStore('店舗2');
		insertStaff(1, '担当者A'); // 店舗1 に 2 人目
		insertStaff(store2, '担当者B');

		// 店舗1 に 1 週間分のスケジュールを入れる（店舗2 は空のまま）.
		const { firstSelectable } = seedWeekSchedules(1, 1);
		...
		const dayBtn = page.locator('.smb-front-day-tile').filter({ hasText: String(d.getDate()) }).first();
		...
		await expect(page.locator('.smb-front-confirm')).toContainText('店舗1');
		await expect(page.locator('.smb-front-confirm')).toContainText('担当者1');
		await expect(page.locator('.smb-front-confirm')).toContainText('10:00');
		await expect(page.locator('.smb-front-confirm')).toContainText('山田 花子');
		await expect(page.locator('.smb-front-confirm')).toContainText('hanako@example.com');
		...
		await expect(page.locator('.smb-front-done__summary')).toContainText('店舗1');
		await expect(page.locator('.smb-front-done__summary')).toContainText('担当者1');
	...
	test('確認画面「修正する」で入力が保持されたままフォームへ戻る', async ({ page }) => {
	...
	test('smb_completion_message を設定すると完了画面に反映される', async ({ page }) => {
		setOption('smb_completion_message', '<p class="custom-done-msg">カスタム完了メッセージです。</p>');
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		...
		await page.getByRole('button', { name: '予約を確定する' }).click();
		await expect(page.getByRole('heading', { name: 'ご予約ありがとうございました' })).toBeVisible({ timeout: 10_000 });
		await expect(page.locator('.smb-front-done__message')).toContainText('カスタム完了メッセージです。');
	});

	// ---- スキップルール: 店舗1・担当者1 ----

	test('スキップ: 店舗1・担当者1 → いきなり日付選択から始まる', async ({ page }) => {
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		await expect(page.getByRole('heading', { name: '日付を選択' })).toBeVisible();
	...
	test('スキップ: 店舗1・担当者2 → 担当者選択から（店舗スキップ）', async ({ page }) => {
		insertStaff(1, '担当者X');
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		await expect(page.getByRole('heading', { name: '担当者を選択' })).toBeVisible();
	...
	test('スキップ: 店舗2・各店舗の担当者1 → 店舗選択後に担当者スキップ', async ({ page }) => {
	...
	test('[smart_booking store_id="X"]: 指定店舗固定・店舗選択スキップ（page_id=7 を一時的に書き換える）', async ({ page }) => {
		...
		try {
			...
			await expect(page.getByRole('button', { name: /担当者1 を選択/ })).toHaveCount(0);
			await expect(page.getByRole('button', { name: /固定担当A を選択/ })).toBeVisible();
			await expect(page.getByRole('button', { name: /固定担当B を選択/ })).toBeVisible();
		} finally {
	...
	test('表示順序 A（デフォルト）: date → time → form の順', async ({ page }) => {
		seedWeekSchedules(1, 1);
	...
	test('表示順序 B: form → date → time の順（設定で flow_order=B）', async ({ page }) => {
	...
	test('時間枠ボタン押下で即次ステップへ自動遷移する（確認ボタン不要）', async ({ page }) => {
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		await page.waitForSelector('.smb-front-day-tile:not(.is-disabled)', { timeout: 10_000 });
		await page.locator('.smb-front-day-tile:not(.is-disabled)').first().click();
		...
		const nextBtn = page.getByRole('button', { name: /次へ|進む/ }).filter({ hasNot: page.locator('text=確認画面へ') });
		// ボタンは「確認画面へ進む」のみフォーム画面で存在。TimeSelect 画面には存在しない.
		await expect(page.locator('.smb-front-time-slots button[type=submit]')).toHaveCount(0);

		await page.getByRole('button', { name: /10:00から11:00/ }).click();
		await expect(page.getByRole('heading', { name: 'お客様情報の入力' })).toBeVisible({ timeout: 5000 });
	});
});
