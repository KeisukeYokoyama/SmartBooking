...
/**
 * 指定した option の値を DB から直接取得する（空文字 / 未設定は null）。
 */
function getOption(key) {
	try {
		const out = execSync(
			`npx wp-env run cli wp db query "SELECT option_value FROM wp_options WHERE option_name='${key}';" --skip-column-names`,
			{
				cwd: path.resolve(__dirname, '..', '..'),
				encoding: 'utf8',
				stdio: ['ignore', 'pipe', 'pipe'],
...
test.describe('Phase 2: 設定画面（5タブ）', () => {
	...
	test('設定ページが React エラーなしで描画される（h1: 設定）', async ({ page }) => {
		await expect(page.locator('h1.smb-page__title')).toHaveText('設定');
		// 5タブが描画されている.
	...
	test('タブ切替が動作する（5タブすべて）', async ({ page }) => {
		...
		await expect(page.getByRole('heading', { name: '予約フロー' })).toBeVisible();

		// メール通知タブへ.
		await page.locator('.smb-tabs [role="tab"]', { hasText: 'メール通知' }).click();
		await expect(page.getByRole('heading', { name: '差出人設定' })).toBeVisible();

		// 外部連携タブへ.
		await page.locator('.smb-tabs [role="tab"]', { hasText: '外部連携' }).click();
		await expect(page.getByRole('heading', { name: 'Googleカレンダー連携' })).toBeVisible();

		// デザインタブへ.
		await page.locator('.smb-tabs [role="tab"]', { hasText: 'デザイン' }).click();
		await expect(page.getByRole('heading', { name: 'カラーカスタマイズ' })).toBeVisible();

		// サポートタブへ.
		await page.locator('.smb-tabs [role="tab"]', { hasText: 'サポート' }).click();
	...
	test('基本設定: 予約フロー順序・カレンダー表示モード・表示期間を保存できる', async ({
		...
		// 予約フロー: フォーム → 日付・時間.
		await page.locator('input[name="smb_booking_flow_order"][value="form-first"]').check();
		// カレンダー表示モード: 月表示.
		await page.locator('input[name="smb_calendar_view_mode"][value="month-grid"]').check();
		// 表示期間: 60.
		await page.locator('select').first().selectOption('60');

		await page.getByRole('button', { name: '基本設定を保存' }).click();
		await expect(page.locator('.smb-toast--success').last()).toContainText('基本設定', {
			timeout: 6000,
		});
	...
	test('基本設定: 予約締切（時間前）を保存すると days=0 として永続化される', async ({ page }) => {
		// 既定で hours. 時間を 5 に.
		...
		const res = await restCall(page, 'GET', 'settings');
		expect(String(res.data.settings.smb_booking_deadline_hours)).toBe('5');
		expect(String(res.data.settings.smb_booking_deadline_days)).toBe('0');
	});

	test('基本設定: 予約締切（日前）に切替→値を保存、hours は 0 に反転する', async ({ page }) => {
		// 日単位に切替.
		await page.locator('input[name="smb_booking_deadline_type"][value="days"]').check();
		...
		const res = await restCall(page, 'GET', 'settings');
		expect(String(res.data.settings.smb_booking_deadline_days)).toBe('3');
		expect(String(res.data.settings.smb_booking_deadline_hours)).toBe('0');
	});

	test('基本設定: 完了メッセージ（HTML許可）を保存できる', async ({ page }) => {
		const msg = 'ご予約ありがとうございます。<br>確認メールをお送りしました。';
		...
		expect(res.data.settings.smb_completion_message).toContain('ご予約ありがとうございます');
	});
	...
	test('メール通知: 差出人名・差出人メール・各種件名を保存できる', async ({ page }) => {
		await page.locator('.smb-tabs [role="tab"]', { hasText: 'メール通知' }).click();
		await expect(page.getByRole('heading', { name: '差出人設定' })).toBeVisible();

		await page.getByLabel('差出人名').fill('テスト予約受付');
		await page.getByLabel('差出人メールアドレス').fill('noreply@example.com');

		// 予約受付（ユーザー宛）件名.
		const receiptUserSubject = page.locator('.smb-settings-section', {
			has: page.getByRole('heading', { name: '予約受付メール（ユーザー宛）' }),
		}).getByLabel('件名');
		await receiptUserSubject.fill('受付メールの件名');

		await page.getByRole('button', { name: 'メール設定を保存' }).click();
		await expect(page.locator('.smb-toast--success').last()).toContainText('メール', {
			timeout: 6000,
		});

		const res = await restCall(page, 'GET', 'settings');
		expect(res.data.settings.smb_mail_from_name).toBe('テスト予約受付');
		expect(res.data.settings.smb_mail_from_email).toBe('noreply@example.com');
		expect(res.data.settings.smb_mail_receipt_user_subject).toBe('受付メールの件名');
	});

	test('メール通知: 不正なメールアドレス形式はサニタイズで空に戻される', async ({ page }) => {
		await page.locator('.smb-tabs [role="tab"]', { hasText: 'メール通知' }).click();
		await page.getByLabel('差出人メールアドレス').fill('not-an-email');
		await page.getByRole('button', { name: 'メール設定を保存' }).click();
		await expect(page.locator('.smb-toast--success').last()).toBeVisible({ timeout: 6000 });
	...
	test('メール通知: テンプレート変数チップで {customer_name} が本文に挿入される', async ({
		...
		await section
			.locator('.smb-var-chip', { has: page.locator('code', { hasText: '{customer_name}' }) })
			.click();
		...
		const res = await restCall(page, 'GET', 'settings');
		expect(res.data.settings.smb_mail_receipt_user_body).toContain('{customer_name}');
	});
	...
	test('外部連携: 初期状態で Google Calendar / ChatWork は OFF', async ({ page }) => {
		await page.locator('.smb-tabs [role="tab"]', { hasText: '外部連携' }).click();
		await expect(page.getByRole('heading', { name: 'Googleカレンダー連携' })).toBeVisible();
		...
		const chatworkSwitch = page
			...
			.getByRole('switch');
		await expect(chatworkSwitch).toHaveAttribute('aria-checked', 'false');
	...
	test('外部連携: ChatWork 注意書き（通知専用アカウント推奨）が表示される', async ({ page }) => {
		await page.locator('.smb-tabs [role="tab"]', { hasText: '外部連携' }).click();
		await expect(
	...
	test('外部連携: Google ON にしてカレンダーIDを保存できる', async ({ page }) => {
		await page.locator('.smb-tabs [role="tab"]', { hasText: '外部連携' }).click();
		await expect(page.getByRole('heading', { name: 'Googleカレンダー連携' })).toBeVisible();
		...
		await expect(googleSection.getByRole('switch')).toHaveAttribute('aria-checked', 'true');

		// カレンダーID 入力.
		await googleSection.getByLabel('カレンダーID').fill('test@group.calendar.google.com');

		await page.getByRole('button', { name: '外部連携の設定を保存' }).click();
		await expect(page.locator('.smb-toast--success').last()).toBeVisible({ timeout: 6000 });

		const res = await restCall(page, 'GET', 'settings');
		// bool は 1/0 で返る.
		expect(String(res.data.settings.smb_google_calendar_enabled)).toBe('1');
		expect(res.data.settings.smb_google_calendar_id).toBe('test@group.calendar.google.com');
	});

	test('外部連携: ON → OFF に戻して保存できる（永続化）', async ({ page }) => {
		// まず ON で保存（REST 直接）.
		...
		// OFF に切替.
		await googleSection.locator('.smb-switch').click();
		await expect(googleSection.getByRole('switch')).toHaveAttribute('aria-checked', 'false');

		await page.getByRole('button', { name: '外部連携の設定を保存' }).click();
		await expect(page.locator('.smb-toast--success').last()).toBeVisible({ timeout: 6000 });

		const res = await restCall(page, 'GET', 'settings');
		expect(String(res.data.settings.smb_google_calendar_enabled)).toBe('0');
	});

	test('外部連携: ChatWork API トークン・ルームID を保存できる', async ({ page }) => {
		await page.locator('.smb-tabs [role="tab"]', { hasText: '外部連携' }).click();
		...
		await expect(chatworkSection.getByRole('switch')).toHaveAttribute('aria-checked', 'true');

		await chatworkSection.getByLabel('APIトークン').fill('test-api-token-xxxx');
		...
		expect(res.data.settings.smb_chatwork_api_token).toBe('test-api-token-xxxx');
	...
	test('デザイン: 不正な HEX 形式はクライアント側で拒否される', async ({ page }) => {
		await page.locator('.smb-tabs [role="tab"]', { hasText: 'デザイン' }).click();
		const buttonHex = page.getByLabel('ボタン色 カラーコード');
		...
		await page.getByRole('button', { name: 'テーマ設定を保存' }).click();
		// エラートースト.
		await expect(page.locator('.smb-toast--error').last()).toContainText(
			/カラーコード|形式/,
			{ timeout: 6000 }
		);
	...
	test('サポート: 使い方ガイド・FAQ・カスタマイズ相談リンクが表示される', async ({ page }) => {
		await page.locator('.smb-tabs [role="tab"]', { hasText: 'サポート' }).click();
		await expect(page.getByRole('heading', { name: 'ヘルプ' })).toBeVisible();

		// 3つのリンク要素.
		await expect(page.locator('text=使い方ガイド').first()).toBeVisible();
		await expect(page.locator('text=よくある質問').first()).toBeVisible();
		await expect(page.getByRole('button', { name: '公式サイトで相談する' })).toBeVisible();

		// リンク先 URL が wp-smart-booking.com.
		const guide = page.locator('a.smb-support-card', { hasText: '使い方ガイド' });
		await expect(guide).toHaveAttribute('href', /wp-smart-booking\.com/);
		const faq = page.locator('a.smb-support-card', { hasText: 'よくある質問' });
		await expect(faq).toHaveAttribute('href', /wp-smart-booking\.com/);
		// 開発元リンク（liberdade-inc.com）も存在.
		await expect(page.locator('a[href*="liberdade-inc.com"]').first()).toBeVisible();
	});
	...
	test('未保存の変更があるタブから別タブに切り替えると警告ダイアログが出る', async ({ page }) => {
		// 基本設定タブで値を変更（ダーティに）.
		await page.locator('input[name="smb_booking_flow_order"][value="form-first"]').check();
	...
	test('REST: 負数の表示期間は int キャストされる（負数も許容される点を記録）', async ({ page }) => {
	...
	test('REST: 不正 HEX（色）はサーバで空に正規化される', async ({ page }) => {
		const res = await restCall(page, 'POST', 'settings', {
	...
	test('REST: 不正メール（差出人）はサーバで空に正規化される', async ({ page }) => {
		const res = await restCall(page, 'POST', 'settings', {
	...
	test('REST: settings が配列でない POST は 400 エラー', async ({ page }) => {
		const res = await restCall(page, 'POST', 'settings', {
	...
	test('メール本文の <script> はサーバで除去される（wp_kses_post）', async ({ page }) => {
		const body = 'hello <script>alert(1)</script> world';
		const res = await restCall(page, 'POST', 'settings', {
			settings: { smb_mail_receipt_user_body: body },
		});
		expect(res.ok).toBe(true);
		expect(res.data.settings.smb_mail_receipt_user_body).not.toContain('<script>');
		expect(res.data.settings.smb_mail_receipt_user_body).toContain('hello');
	});
	...
	test('REST: bool 切替（1 <-> 0）が永続化される', async ({ page }) => {
		...
		expect(String(get.data.settings.smb_google_calendar_enabled)).toBe('1');
		await restCall(page, 'POST', 'settings', {
			settings: { smb_google_calendar_enabled: 0 },
		});
		get = await restCall(page, 'GET', 'settings');
		expect(String(get.data.settings.smb_google_calendar_enabled)).toBe('0');
	});
});
...
test.describe('Phase 2: 設定画面 - レスポンシブ（375px）', () => {
	...
	test('スマホ幅で 5 タブ切替・基本設定の保存ができる', async ({ page }) => {
		resetSettings();
		await bootstrapAdmin(page, 'settings');
		await page.waitForSelector('.smb-page--settings', { timeout: 15000 });

		// 5 タブすべて切替できる.
		const tabNames = ['基本設定', 'メール通知', '外部連携', 'デザイン', 'サポート'];
		for (const name of tabNames) {
			await page.locator('.smb-tabs [role="tab"]', { hasText: name }).click();
			await expect(
		...
		// 基本設定タブに戻って保存.
		await page.locator('.smb-tabs [role="tab"]', { hasText: '基本設定' }).click();
		await page.locator('input[name="smb_calendar_view_mode"][value="month-grid"]').check();
		await page.getByRole('button', { name: '基本設定を保存' }).click();
		await expect(page.locator('.smb-toast--success').last()).toBeVisible({ timeout: 6000 });
	});
