...
test.describe( 'Gen-B リグレッション: 重複登録防止 + 隣月対応 + テーブル化', () => {
	...
	test( '重複登録防止: 同一 (店舗,担当者,日付,開始時刻) は created -> updated に変わる', async ( {
		...
		const out = dbQuery(
			`SELECT COUNT(*), MAX(capacity) FROM wp_smb_schedules WHERE schedule_date='${ targetDate }' AND start_time='10:00:00';`
		);
	...
	test( '予約付き既存枠: 重複 POST は skipped に分類され capacity が変わらない', async ( {
		...
		dbQuery(
			`INSERT INTO wp_smb_schedules (store_id, staff_id, schedule_date, start_time, end_time, capacity, booked_count, is_active, created_at, updated_at) VALUES (${ USER_STORE_ID }, ${ USER_STAFF_ID }, '${ targetDate }', '14:00:00', '15:00:00', 1, 1, 1, NOW(), NOW());`
		);
		...
		const out = dbQuery(
			`SELECT capacity, booked_count FROM wp_smb_schedules WHERE schedule_date='${ targetDate }' AND start_time='14:00:00';`
		);
		const m = out.match( /(\d+)\s+(\d+)/ );
		expect( m ).not.toBeNull();
		expect( Number( m[ 1 ] ) ).toBe( 1 );
	...
	test( 'UI: 既存登録済み時間は追加モーダル内でグレーアウト + 警告表示', async ( { page } ) => {
		...
		const out = dbQuery(
			`SELECT COUNT(*) FROM wp_smb_schedules WHERE schedule_date='${ targetDate }';`
		);
		const m = out.match( /(\d+)/ );
		expect( m ).not.toBeNull();
		...
		const cap10 = dbQuery(
			`SELECT capacity FROM wp_smb_schedules WHERE schedule_date='${ targetDate }' AND start_time='10:00:00';`
		);
	...
	test( '隣月日付: スケジュール存在時は is-other-month のまま残数バッジ表示', async ( {
		...
		dbQuery(
			`INSERT INTO wp_smb_schedules (store_id, staff_id, schedule_date, start_time, end_time, capacity, booked_count, is_active, created_at, updated_at) VALUES (${ USER_STORE_ID }, ${ USER_STAFF_ID }, '${ otherYmd }', '13:00:00', '14:00:00', 3, 0, 1, NOW(), NOW());`
		);
