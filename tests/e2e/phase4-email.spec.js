...
test.describe( 'Phase 4 Eval-A: Email 連携', () => {
	...
	test.beforeEach( async () => {
		...
		setOptionRaw( 'smart_booking_mail_from_name', 'Smart Booking Test' );
	...
	// ----------------------------------------------------------------
	test( '承認時: ユーザー宛 1 通 (件名は smart_booking_mail_approval_user_subject)', async ( {
		page,
	} ) => {
		setStoreEmail( USER_STORE_ID, 'store-a@example.com' );
		setStaffEmail( USER_STAFF_ID, 'staff-a@example.com' );
	...
	// ----------------------------------------------------------------
	test( 'smart_booking_mail_from_name / smart_booking_mail_from_email が空でも送信は成功する', async ( {
		page,
	} ) => {
		setStoreEmail( USER_STORE_ID, 'store-a@example.com' );
		setStaffEmail( USER_STAFF_ID, '' );
		setOptionSql( 'smart_booking_mail_from_name', "''" );
		setOptionSql( 'smart_booking_mail_from_email', "''" );
