...
test.describe( 'Phase 1: 管理画面メニューとページ描画', () => {
	...
	for ( const p of ADMIN_PAGES ) {
		test( `2-2. サブメニューページ [${ p.label }] が React マウント領域を出力する`, async ( {
			...
			expect(
				fatal,
				`Console errors on ${ p.slug }:\n${ fatal.join( '\n' ) }`
			).toEqual( [] );
		} );
	}

	test( '2-3. 5 サブメニューすべてが DOM に登録されている', async ( {
		page,
	} ) => {
		...
		for ( const slug of expectedSlugs ) {
			const links = page.locator(
				`#toplevel_page_smart-booking .wp-submenu a`
			);
			...
			expect(
				matched.length,
				`submenu link for ${ slug }: matched=${ JSON.stringify(
					hrefs
				) }`
			).toBe( 1 );
		}
	...
	test( '2-4. メニューアイコンに dashicons-calendar-alt クラスが付与されている', async ( {
		...
		const icon = page.locator(
			'#toplevel_page_smart-booking .wp-menu-image'
		);
...
test.describe( 'Phase 1: REST API', () => {
	...
	test( '3-3. 管理画面ログイン状態 + X-WP-Nonce 付きで /stores の GET が 200 を返す', async ( {
		...
		const ctx = await page.evaluate( () => {
			...
			if ( typeof window.smartBookingAdmin === 'undefined' ) {
				return null;
			}
			return {
	...
	for ( const resource of RESOURCES ) {
		test( `3-4. 非ログイン状態で /${ resource } の GET/POST が拒否される`, async ( {
			...
			const getRes = await request.get(
				`/wp-json/smart-booking/v1/${ resource }`
			);
			expect(
				[ 401, 403 ],
		...
		test( `3-5. 非ログイン状態で /${ resource }/1 の PUT/DELETE が拒否される`, async ( {
			...
			const putRes = await request.put(
				`/wp-json/smart-booking/v1/${ resource }/1`,
				{
					data: {},
				}
			);
	...
	test( '3-6. 非ログイン状態で /settings の GET/POST が拒否される', async ( {
		request,
	} ) => {
		const getRes = await request.get(
