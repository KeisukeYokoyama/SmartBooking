...
async function seedReservations(page, n = 3, dateOffset = 2) {
	...
	if (!schedRes.ok) throw new Error('seed schedule failed: ' + JSON.stringify(schedRes.data));
	...
	for (let i = 0; i < n; i++) {
		...
		if (!r.ok) throw new Error('seed reservation failed: ' + JSON.stringify(r.data));
	}
...
test.describe('Phase 2: 予約一覧 拡張', () => {
	...
	test('予約が3件あるとテーブル（デスクトップ）またはカード（モバイル）に表示される', async ({ page, isMobile }, testInfo) => {
		await seedReservations(page, 3);
		await page.reload();
		await page.waitForSelector('.smb-page--reservations', { timeout: 15000 });
		const mobile = testInfo.project.name === 'mobile';
		if (mobile) {
			await expect(page.locator('.smb-reservation-card')).toHaveCount(3);
		} else {
	...
	test('名前フィルタで絞り込める', async ({ page }, testInfo) => {
		...
		const mobile = testInfo.project.name === 'mobile';
		if (mobile) {
			await expect(page.locator('.smb-reservation-card')).toHaveCount(1);
		} else {
			await expect(page.locator('.smb-table__row')).toHaveCount(1);
		}
	});

	test('ステータスフィルタで承認待ちのみを抽出', async ({ page }, testInfo) => {
		await seedReservations(page, 3);
		await page.reload();
		await page.waitForSelector('.smb-page--reservations', { timeout: 15000 });
		// ステータス select はグリッド内の 4番目（名前、メール、店舗、担当者、日付2つを経て最後）。
		// ラベル「ステータス」を持つラッパで特定する.
		const statusField = page.locator('.smb-field', { has: page.locator('label', { hasText: 'ステータス' }) });
	...
	test('承認ボタンでステータスが承認済みに変わる（楽観更新）', async ({ page }, testInfo) => {
		// pending の予約を1件だけ作る.
		const d = ymd(2);
		const schedRes = await restCall(page, 'POST', 'schedules', {
			items: [
		...
		if (mobile) {
			// モバイルではカード内の承認ボタン.
			const card = page.locator('.smb-reservation-card', { hasText: '承認テスト' });
			await card.getByRole('button', { name: '承認', exact: true }).click();
			await expect(page.locator('.smb-toast--success').last()).toContainText('承認', { timeout: 5000 });
		} else {
			const row = page.locator('.smb-table__row', { hasText: '承認テスト' });
			await row.getByRole('button', { name: '承認', exact: true }).click();
			await expect(page.locator('.smb-toast--success').last()).toContainText('承認', { timeout: 5000 });
		}
	});

	test('予約詳細モーダルが開く（テーブル / カードから）', async ({ page }, testInfo) => {
		...
		await expect(page.locator('.smb-modal__title').first()).toContainText(/予約/);
	});
	...
	test('CSV エクスポートでファイルがダウンロードされる', async ({ page }) => {
		...
		// 実態: content = `"﻿予約..."` のような JSON エンコード文字列（二重エスケープ）。
		const clean = content.replace(/^﻿/, '').replace(/^﻿/, '');
		expect(clean, 'CSV は生テキストである必要があります（JSON 文字列化されてはいけない）').toContain('予約者');
		expect(clean).toContain('予約者1');
		expect(clean).toContain('予約者2');
	});

	test('手動予約作成: 満席の枠は選択不可（is-disabled）', async ({ page }) => {
		...
		await expect(page.locator('.smb-slot-btn').first()).toHaveClass(/is-disabled/);
		await expect(page.locator('.smb-slot-btn').first()).toContainText('満席');
	});
