...
test.describe( 'Phase 2: 予約一覧 - スモーク', () => {
	...
	test.beforeEach( async () => {
		...
		try {
			wpCli(
				'db query "DELETE FROM wp_smb_reservation_meta;" --skip-column-names'
			);
			wpCli(
				'db query "DELETE FROM wp_smb_reservations;" --skip-column-names'
			);
			wpCli(
				'db query "DELETE FROM wp_smb_schedules;" --skip-column-names'
			);
		} catch ( _e ) {
			// noop.
		}
