...
const HIT_JSON = JSON.stringify( {
	message: null,
	results: [
		{ address1: '東京都', address2: '渋谷区', address3: '渋谷', kana1: 'ﾄｳｷｮｳﾄ', kana2: 'ｼﾌﾞﾔｸ', kana3: 'ｼﾌﾞﾔ', prefcode: '13', zipcode: '1500002' },
	],
	status: 200,
} );
const EMPTY_JSON = JSON.stringify( { message: null, results: null, status: 200 } );
...
function insertAddressField( key, label, { autofill = true, required = 0, sort = 60, cond_key = null, cond_val = null } = {} ) {
...
function metaCount( metaKey ) {
	const out = dbq( `SELECT COUNT(*) FROM ${ RM } WHERE meta_key='${ metaKey }';`, '--skip-column-names' );
	const m = /(\d+)/.exec( out );
	return m ? parseInt( m[ 1 ], 10 ) : -1;
}
function latestReservationId() {
	const out = dbq( `SELECT id FROM wp_smart_booking_reservations ORDER BY id DESC LIMIT 1;`, '--skip-column-names' );
	const m = /(\d+)/.exec( out );
...
test.describe( 'v0.3.0 ④: 住所フィールド', () => {
	...
	async function pickDateTime( page, zipSelector ) {
		...
		await page.waitForSelector( '.smb-front-day-tile:not(.is-disabled)', { timeout: 15_000 } );
	...
	test( 'A: 自動補完（全角→半角正規化・住所補完・2キー保存）', async ( { page } ) => {
		insertAddressField( 'dest', 'お届け先住所', { autofill: true, required: 0, sort: 60 } );
		await page.route( '**/zipcloud.ibsnet.co.jp/**', ( route ) =>
			route.fulfill( { status: 200, contentType: 'application/json', body: HIT_JSON } )
		);
		await pickDateTime( page, '#smb-front-field-dest-zip' );

		// 全角郵便番号を入力 → デバウンス後に住所が補完される。
		await page.locator( '#smb-front-field-dest-zip' ).fill( '１５００００２' );
		await expect( page.locator( '#smb-front-field-dest-address' ) ).toHaveValue( '東京都渋谷区渋谷', { timeout: 8_000 } );

		// 番地を追記（手編集）。
		await page.locator( '#smb-front-field-dest-address' ).fill( '東京都渋谷区渋谷1-2-3' );

		await page.locator( '#smb-front-field-customer_name' ).fill( '住所 太郎' );
		await page.locator( '#smb-front-field-customer_email' ).fill( 'addr@example.com' );
		await page.locator( '#smb-front-field-customer_phone' ).fill( '090-1111-2222' );
		await page.getByRole( 'button', { name: '予約内容の確認' } ).click();
		await page.waitForSelector( '.smb-front-confirm-page, .smb-front-confirm', { timeout: 10_000 } );
		// 確認画面に 〒1500002 住所 が表示される。
		await expect( page.getByText( '〒1500002 東京都渋谷区渋谷1-2-3' ) ).toBeVisible();
		...
		expect( metaVal( rid, 'dest_address' ) ).toBe( '東京都渋谷区渋谷1-2-3' );
	} );

	test( 'B: フェイルソフト（0件→補完なし・手入力で完走・console エラー無し）', async ( { page } ) => {
		insertAddressField( 'dest', 'お届け先住所', { autofill: true, required: 0, sort: 60 } );
		const consoleErrors = [];
		const pageErrors = [];
		...
		await expect( page.locator( '#smb-front-field-dest-address' ) ).toHaveValue( '' );

		// 手入力で完走。
		await page.locator( '#smb-front-field-dest-address' ).fill( '東京都テスト区9-9-9' );
		await page.locator( '#smb-front-field-customer_name' ).fill( 'フェイル 太郎' );
		await page.locator( '#smb-front-field-customer_email' ).fill( 'fs@example.com' );
		await page.locator( '#smb-front-field-customer_phone' ).fill( '090-3333-4444' );
		await page.getByRole( 'button', { name: '予約内容の確認' } ).click();
		await page.waitForSelector( '.smb-front-confirm-page, .smb-front-confirm', { timeout: 10_000 } );
		await page.getByRole( 'button', { name: '予約を確定する' } ).click();
		await expect( page.getByRole( 'heading', { name: 'ご予約ありがとうございました' } ) ).toBeVisible( { timeout: 10_000 } );
	...
	test( 'C: ③連携（希望する で出現/入力 → 希望しない で破棄）', async ( { page } ) => {
		insertRadio( 'shiryo', '資料送付', [ '希望する', '希望しない' ], 50 );
		insertAddressField( 'dest', '送付先住所', { autofill: true, required: 0, sort: 60, cond_key: 'shiryo', cond_val: '希望する' } );
		await page.route( '**/zipcloud.ibsnet.co.jp/**', ( route ) =>
			route.fulfill( { status: 200, contentType: 'application/json', body: HIT_JSON } )
		);
		...
		// 初期: 子(住所)は非表示。
		await expect( page.locator( '#smb-front-field-dest-zip' ) ).toHaveCount( 0 );
		// 希望する → 出現。
		await page.locator( '#smb-front-field-shiryo-opt-0' ).check();
		await expect( page.locator( '#smb-front-field-dest-zip' ) ).toBeVisible();
		await page.locator( '#smb-front-field-dest-zip' ).fill( '1500002' );
		await page.locator( '#smb-front-field-dest-address' ).fill( '東京都渋谷区渋谷5-5-5' );
		// 希望しない → 消失。
		await page.locator( '#smb-front-field-shiryo-opt-1' ).check();
		await expect( page.locator( '#smb-front-field-dest-zip' ) ).toHaveCount( 0 );

		await page.locator( '#smb-front-field-customer_name' ).fill( '条件 太郎' );
		await page.locator( '#smb-front-field-customer_email' ).fill( 'cond@example.com' );
		await page.locator( '#smb-front-field-customer_phone' ).fill( '090-5555-6666' );
		await page.getByRole( 'button', { name: '予約内容の確認' } ).click();
		await page.waitForSelector( '.smb-front-confirm-page, .smb-front-confirm', { timeout: 10_000 } );
		await page.getByRole( 'button', { name: '予約を確定する' } ).click();
		await expect( page.getByRole( 'heading', { name: 'ご予約ありがとうございました' } ) ).toBeVisible( { timeout: 10_000 } );
	...
	test( 'D: 自動入力OFF無通信 ＋ 直接POSTバリデーション', async ( { page } ) => {
		insertAddressField( 'destoff', '住所OFF', { autofill: false, required: 1, sort: 60 } );
		insertAddressField( 'destopt', '住所任意', { autofill: true, required: 0, sort: 61 } );

		const zipcloudHits = [];
		page.on( 'request', ( req ) => { if ( req.url().includes( 'zipcloud' ) ) zipcloudHits.push( req.url() ); } );
		...
		// 必須・両方空 → 400 required。
		const r1 = await publicRest( page, 'public/reservations', { method: 'POST', body: { ...base, custom_fields: { destoff: { zip: '', address: '' } } } } );
		expect( r1.status ).toBe( 400 );
		expect( r1.data.code ).toBe( 'smb_reservation_custom_field_required' );
		// 必須・郵便番号のみ（住所空）→ 400 required。
		const r2 = await publicRest( page, 'public/reservations', { method: 'POST', body: { ...base, custom_fields: { destoff: { zip: '1500002', address: '' } } } } );
		expect( r2.status ).toBe( 400 );
		expect( r2.data.code ).toBe( 'smb_reservation_custom_field_required' );
		// 必須・郵便番号が7桁でない → 400 zip_invalid。
		const r3 = await publicRest( page, 'public/reservations', { method: 'POST', body: { ...base, custom_fields: { destoff: { zip: '123', address: '住所X' } } } } );
		expect( r3.status ).toBe( 400 );
		expect( r3.data.code ).toBe( 'smb_reservation_zip_invalid' );
		// 任意フィールドで zip 非空・非7桁 → 400 zip_invalid（必須は満たす）。
		const r4 = await publicRest( page, 'public/reservations', { method: 'POST', body: { ...base, custom_fields: { destoff: { zip: '1500002', address: '住所X' }, destopt: { zip: '99', address: '' } } } } );
		expect( r4.status ).toBe( 400 );
		expect( r4.data.code ).toBe( 'smb_reservation_zip_invalid' );
		// 妥当 → 200。
		const r5 = await publicRest( page, 'public/reservations', { method: 'POST', body: { ...base, custom_fields: { destoff: { zip: '1500002', address: '住所X' } } } } );
