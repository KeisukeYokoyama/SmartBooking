			...
			const qs = query
				? '?' +
					Object.entries(query)
						...
						.join('&')
				: '';
			const url = ctx.restUrl.replace(/\/$/, '') + '/' + endpoint.replace(/^\//, '') + qs;
			const init = {
				method,
...
async function bootstrapAdmin(page, pageKey = 'schedule') {
	...
	await page.waitForFunction(() => !!window.smartBookingAdmin?.nonce, { timeout: 15000 });
}
...
function insertStoreDirectly({ name, calendar_color = '#2271b1', is_active = 1, sort_order = 20 }) {
 ...
 */
function insertStaffDirectly({ store_id, name, is_active = 1, sort_order = 20 }) {
	const sql = `INSERT INTO wp_smb_staff (store_id, name, email, phone, description, image_id, sort_order, is_active, created_at, updated_at) VALUES (${store_id}, '${name.replace(/'/g, "''")}', '', '', '', 0, ${sort_order}, ${is_active}, NOW(), NOW());`;
	execSync(`npx wp-env run cli wp db query "${sql}"`, {
		cwd: path.resolve(__dirname, '..', '..'),
		encoding: 'utf8',
		stdio: ['ignore', 'pipe', 'pipe'],
