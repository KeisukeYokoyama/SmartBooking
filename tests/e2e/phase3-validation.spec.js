...
function setBookedCount( scheduleId, count ) {
	...
	const path = require( 'node:path' );
	execSync(
		`npx wp-env run cli wp db query "UPDATE wp_smart_booking_schedules SET booked_count = ${ count } WHERE id = ${ scheduleId };"`,
		{
			cwd: path.resolve( __dirname, '..', '..' ),
			encoding: 'utf8',
			stdio: [ 'ignore', 'pipe', 'pipe' ],
...
function deactivateSchedule( scheduleId ) {
	...
	const path = require( 'node:path' );
	execSync(
		`npx wp-env run cli wp db query "UPDATE wp_smart_booking_schedules SET is_active = 0 WHERE id = ${ scheduleId };"`,
		{
			cwd: path.resolve( __dirname, '..', '..' ),
			encoding: 'utf8',
			stdio: [ 'ignore', 'pipe', 'pipe' ],
...
test.describe( 'Phase 3 Eval-3: フロント予約フォーム 異常系', () => {
	...
	test( 'REST 直叩き: capacity 超過の予約は 409 smb_reservation_full', async ( {
		...
		expect( countRows( 'wp_smart_booking_reservations' ) ).toBe( 0 );
	} );
	...
	test( 'deadline_days=10: 1日後の枠タイルは disabled で「締切」バッジ', async ( {
		...
		setOption( 'smart_booking_calendar_view_mode', 'day_only' );
		setOption( 'smart_booking_booking_deadline_days', 10 );
		const d = ymd( 1 );
		insertSchedule( {
			storeId: USER_STORE_ID,
			staffId: USER_STAFF_ID,
	...
	test( '競合: capacity=1 に並列 POST 2 件 → 1 件成功・もう 1 件 409', async ( {
		...
		expect( countRows( 'wp_smart_booking_reservations' ) ).toBe( 1 );
