...
test.describe('Phase 3 Eval-3: フロント予約フォーム 異常系', () => {
	...
	test('必須3フィールドすべて空で送信 → エラー表示が出て次ステップに進まない', async ({ page }) => {
		insertSchedule({
			storeId: 1,
			staffId: 1,
			date: ymd(1),
	...
	test('氏名のみ入力 → メール・電話に必須エラー、氏名は OK', async ({ page }) => {
		insertSchedule({
			storeId: 1,
			staffId: 1,
			date: ymd(1),
		...
		await page.locator('#smb-front-field-customer_name').fill('テスト 太郎');
		await page.getByRole('button', { name: '確認画面へ進む' }).click();
		...
		await expect(page.locator('#smb-front-field-customer_name')).toHaveAttribute('aria-invalid', 'false');
		await expect(page.locator('#smb-front-field-customer_email')).toHaveAttribute('aria-invalid', 'true');
		await expect(page.locator('#smb-front-field-customer_phone')).toHaveAttribute('aria-invalid', 'true');

		// フォーカスは「最初のエラーがあるフィールド」（DOM 上で 2 つの空フィールドのうち先頭）.
		const focusedId = await page.evaluate(() => document.activeElement && document.activeElement.id);
		// customer_email か customer_phone のいずれか（baseline の sort_order 次第）.
		expect(['smb-front-field-customer_email', 'smb-front-field-customer_phone']).toContain(focusedId);
		// 氏名以外の input のうち DOM 先頭のもの = focused.
		const firstNonNameInputId = await page
			.locator('.smb-front-form input.smb-front-form__input:not(#smb-front-field-customer_name)')
			.first()
			.getAttribute('id');
		expect(focusedId).toBe(firstNonNameInputId);
	});

	test('メール形式不正 (abc, abc@, @abc.com) で送信 → 形式エラー、送信されない', async ({ page }) => {
		insertSchedule({
			storeId: 1,
			staffId: 1,
			date: ymd(1),
		...
		const invalidEmails = ['abc', 'abc@', '@abc.com', 'abc def@example.com'];
		for (const bad of invalidEmails) {
			...
			// メールフィールドにエラーが出ている.
			const emailErr = page.locator('#smb-front-field-customer_email-err');
	...
	test('電話番号に数字以外（英字）が混入 → フォーマットエラー、送信されない', async ({ page }) => {
		insertSchedule({
			storeId: 1,
			staffId: 1,
			date: ymd(1),
		...
		await page.locator('#smb-front-field-customer_name').fill('テスト');
		await page.locator('#smb-front-field-customer_email').fill('test@example.com');
		await page.locator('#smb-front-field-customer_phone').fill('phone-bad-1234');
		await page.getByRole('button', { name: '確認画面へ進む' }).click();
	...
	test('カスタムフィールド is_required=1 が空 → 必須エラー、送信されない', async ({ page }) => {
		insertSchedule({
			storeId: 1,
			staffId: 1,
			date: ymd(1),
	...
	test('満席の時間枠ボタンは disabled でクリック不可', async ({ page }) => {
		...
		// 満席ボタン (10:00).
		const fullBtn = page.getByRole('button', { name: /^10:00から11:00 満席 選択不可$/ });
		await expect(fullBtn).toBeDisabled();

		// クリックを試みる（force:true）→ 状態変化なし.
		await fullBtn.click({ force: true }).catch(() => {});
		await expect(page.getByRole('heading', { name: 'お客様情報の入力' })).toHaveCount(0);
	});

	test('REST 直叩き: capacity 超過の予約は 409 smb_reservation_full', async ({ page }) => {
		const d = ymd(1);
		const fullSchedId = insertSchedule({
			storeId: 1,
			staffId: 1,
	...
	test('deadline_days=10: 1日後の枠タイルは disabled で「締切」バッジ', async ({ page }) => {
		setOption('smb_calendar_view_mode', 'day_only');
	...
	test('REST 直叩き: 締切超過の schedule に POST → 400 smb_reservation_deadline_passed', async ({ page }) => {
		setOption('smb_booking_deadline_days', 10);
		const d = ymd(1);
		...
		expect(res.data && res.data.code).toBe('smb_reservation_deadline_passed');
		expect(countRows('wp_smb_reservations')).toBe(0);
	...
	test('競合: capacity=1 に並列 POST 2 件 → 1 件成功・もう 1 件 409', async ({ page }) => {
		const d = ymd(1);
		const sid = insertSchedule({
			storeId: 1,
			staffId: 1,
		...
		const results = await page.evaluate(
			async ({ scheduleId }) => {
				...
				const url = ctx.restUrl.replace(/\/$/, '') + '/public/reservations';
				const headers = {
				...
				return Promise.all([makeReq('競合A', 'a@example.com'), makeReq('競合B', 'b@example.com')]);
			},
			{ scheduleId: sid },
		);
	...
	test('ハニーポット: 値ありで POST → 400 smb_reservation_spam_rejected', async ({ page }) => {
		const d = ymd(1);
		const sid = insertSchedule({
			storeId: 1,
			staffId: 1,
		...
		expect(res.status).toBe(400);
		expect(res.data && res.data.code).toBe('smb_reservation_spam_rejected');
	...
	test('REST 直叩き: 存在しない schedule_id → 400 smb_reservation_schedule_not_found', async ({ page }) => {
		await gotoFrontForm(page);
		const res = await publicRest(page, 'public/reservations', {
			method: 'POST',
			body: {
		...
		expect(res.status).toBe(400);
		expect(res.data && res.data.code).toBe('smb_reservation_schedule_not_found');
		expect(countRows('wp_smb_reservations')).toBe(0);
	});

	test('REST 直叩き: schedule_id 欠如 → 400 smb_reservation_schedule_required', async ({ page }) => {
		await gotoFrontForm(page);
		const res = await publicRest(page, 'public/reservations', {
			method: 'POST',
			body: {
		...
		expect(res.status).toBe(400);
		expect(res.data && res.data.code).toBe('smb_reservation_schedule_required');
		expect(countRows('wp_smb_reservations')).toBe(0);
	});

	test('REST 直叩き: is_active=0 の schedule → 400 smb_reservation_schedule_not_found', async ({ page }) => {
		const d = ymd(1);
		const sid = insertSchedule({
			storeId: 1,
			staffId: 1,
	...
	test('REST 直叩き: 必須3フィールド (氏名/メール/電話) のいずれか欠如 → 400', async ({ page }) => {
		const d = ymd(1);
		const sid = insertSchedule({
			storeId: 1,
			staffId: 1,
		...
		expect(r1.data && r1.data.code).toBe('smb_reservation_name_required');
		...
		expect(r2.data && r2.data.code).toBe('smb_reservation_email_required');
		...
		expect(r3.data && r3.data.code).toBe('smb_reservation_phone_required');
		...
		expect(r4.data && r4.data.code).toBe('smb_reservation_email_invalid');
	...
	test('確認画面で送信失敗 (409 満席) → エラー表示 + ステップは confirm に留まる + フォーカス遷移', async ({
		...
		await expect(page.getByRole('heading', { name: 'ご予約ありがとうございました' })).toHaveCount(0);
		...
		await expect(page.getByRole('button', { name: '予約を確定する' })).toBeEnabled();

		// DB 検証: 競合検出のためレコード追加されない.
		expect(countRows('wp_smb_reservations')).toBe(0);
	});

	test('過去枠 (現在時刻より前) には POST できない: 400 smb_reservation_closed', async ({ page }) => {
		...
		expect(res.status).toBe(400);
		expect(['smb_reservation_closed', 'smb_reservation_deadline_passed']).toContain(
