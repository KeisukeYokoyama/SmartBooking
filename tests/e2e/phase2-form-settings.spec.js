...
function restoreThemeColors() {
	try {
		execSync(
			`npx wp-env run cli wp db query "DELETE FROM wp_options WHERE option_name IN ('smart_booking_color_button','smart_booking_color_date_selected','smart_booking_color_time_selected','smart_booking_color_required_mark','smart_booking_color_focus');"`,
			{
...
test.describe( 'Phase 2: フォーム設定（フィールド）', () => {
	...
	test( 'フィールド設定タブに初期3フィールド（氏名/メール/電話）がロック表示される', async ( {
		page,
	} ) => {
		...
		await expect(
			page.locator( '.smb-field-list__row.is-protected' )
		).toHaveCount( 3 );
	} );

	test( '初期フィールドの削除ボタンは無効化されている', async ( {
		page,
	} ) => {
	...
	test( '新規フィールド（text型）をフィールドタイプカードから追加できる', async ( {
		page,
	} ) => {
		...
		await page
			...
			.click();
		await expect(
			page.locator( '.smb-modal__title', { hasText: 'フィールドを追加' } )
		).toBeVisible();
		await page
	...
	test( 'select 型は選択肢未入力で追加するとバリデーションエラー', async ( {
		page,
	} ) => {
		await page
	...
	test( '追加したフィールドを編集・削除できる', async ( { page } ) => {
		...
		await page
			...
			.click();
		await expect(
			page.locator( '.smb-modal__title', { hasText: 'フィールドを編集' } )
		).toBeVisible();
		await page
			.getByLabel( /ラベル/, { exact: false } )
			.first()
	...
	test( '初期フィールド customer_name を DELETE するとAPIが 400 を返す（保護）', async ( {
		page,
	} ) => {
		...
		const target = list.data.find(
			( f ) => f.field_key === 'customer_name'
		);
		expect( target ).toBeTruthy();
		const delRes = await restCall(
			page,
			'DELETE',
			`custom-fields/${ target.id }`
		);
	...
	test( '初期フィールドはタイプ変更 / 必須解除しても結果的に元の値が保持される', async ( {
		page,
	} ) => {
		const list = await restCall( page, 'GET', 'custom-fields' );
		const target = list.data.find(
