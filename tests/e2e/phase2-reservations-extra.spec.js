...
async function seedSchedule(page, dateOffset = 3, capacity = 50) {
	...
	if (!r.ok) throw new Error('schedule seed failed: ' + JSON.stringify(r.data));
...
test.describe('Phase 2: 予約一覧 追加カバレッジ', () => {
	...
	test('21件の予約があるとページネーションが2ページ分表示される', async ({ page }) => {
		...
		await expect(page.locator('.smb-pagination__summary')).toContainText('21');
		// 2 ページ目ボタンが押せる.
		await page.getByRole('button', { name: '2 ページ目' }).click();
		// summary が "21 件中 21–21 件" になる.
		await expect(page.locator('.smb-pagination__summary')).toContainText('21–21');
	});

	test('カスタムフィールドありの予約: 詳細モーダルにカスタム入力値が出る', async ({ page }, testInfo) => {
		...
		if (!cf.ok) throw new Error('custom field create failed: ' + JSON.stringify(cf.data));
		...
		if (!res.ok) throw new Error('reservation create failed: ' + JSON.stringify(res.data));
		...
		if (mobile) {
			await page.locator('.smb-reservation-card', { hasText: 'カスタム太郎' }).click();
		} else {
			await page
		...
		await expect(page.locator('.smb-modal')).toContainText('リベルダージ株式会社');
	});

	test('CSV: 名前フィルタ適用時、フィルタにマッチした予約のみ出力される', async ({ page }) => {
