...
async function gotoFrontForm(page) {
	...
	await page.waitForSelector('.smb-front-root .smb-front-loading', { state: 'detached', timeout: 15_000 }).catch(() => {});
}
 ...
 */
function insertStore(name, { is_active = 1, sort_order = 20, calendar_color = '#2271b1' } = {}) {
	const sql = `INSERT INTO wp_smb_stores (name, phone, email, prefecture, city, address_line, description, image_id, calendar_color, is_active, sort_order, created_at, updated_at) VALUES ('${name.replace(/'/g, "''")}', '', '', '', '', '', '', 0, '${calendar_color}', ${is_active}, ${sort_order}, NOW(), NOW());`;
	wpCli(`wp db query "${sql}"`);
	const out = wpCli(`wp db query "SELECT MAX(id) FROM wp_smb_stores;" --skip-column-names`);
	const m = /(\d+)/.exec(out);
...
function insertStaff(storeId, name, { is_active = 1, sort_order = 20 } = {}) {
	const sql = `INSERT INTO wp_smb_staff (store_id, name, email, phone, description, image_id, sort_order, is_active, created_at, updated_at) VALUES (${storeId}, '${name.replace(/'/g, "''")}', '', '', '', 0, ${sort_order}, ${is_active}, NOW(), NOW());`;
	wpCli(`wp db query "${sql}"`);
	const out = wpCli(`wp db query "SELECT MAX(id) FROM wp_smb_staff;" --skip-column-names`);
	const m = /(\d+)/.exec(out);
	return m ? parseInt(m[1], 10) : 0;
}
 ...
 */
function insertSchedule({ storeId, staffId, date, start, end, capacity = 3 }) {
	...
	wpCli(`wp db query "${sql}"`);
	const out = wpCli(`wp db query "SELECT MAX(id) FROM wp_smb_schedules;" --skip-column-names`);
	const m = /(\d+)/.exec(out);
	return m ? parseInt(m[1], 10) : 0;
}
...
function insertSchedulesBulk(rows) {
	...
	const values = rows
		.map(
			(r) =>
				`(${r.storeId}, ${r.staffId}, '${r.date}', '${r.start}', '${r.end}', ${r.capacity || 3}, 0, 1, NOW(), NOW())`,
		)
		.join(', ');
...
function countRows(table) {
	const out = wpCli(`wp db query "SELECT COUNT(*) FROM ${table};" --skip-column-names`);
	const m = /(\d+)/.exec(out);
...
async function publicRest(page, endpoint, opts = {}) {
	...
	return page.evaluate(
			...
			const qs = query
				? '?' +
					Object.entries(query)
						...
						.join('&')
				: '';
			const url = restUrl.replace(/\/$/, '') + '/' + endpoint.replace(/^\//, '') + qs;
			const headers = { Accept: 'application/json' };
			if (sendNonce) headers['X-WP-Nonce'] = ctx.nonce;
		...
		{ endpoint, method, body: opts.body || null, query: opts.query || null, sendNonce },
	);
}
 ...
 */
async function fillCoreFormAndGoConfirm(page, { name = 'テスト 太郎', email = 'test@example.com', phone = '090-0000-0000' } = {}) {
