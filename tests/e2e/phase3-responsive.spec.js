...
function seedWeekSchedules(storeId, staffId) {
	...
	for (let i = 1; i <= 6; i++) {
		const d = ymd(i);
		rows.push({ storeId, staffId, date: d, start: '10:00:00', end: '11:00:00', capacity: 3 });
		rows.push({ storeId, staffId, date: d, start: '14:00:00', end: '15:00:00', capacity: 3 });
	}
...
test.describe('Phase 3 Eval-4: レスポンシブ + アクセシビリティ', () => {
	...
	test('375px: ルートが横スクロールせず、カードが縦並びで表示される', async ({ page }) => {
		await page.setViewportSize({ width: 375, height: 667 });
		// 店舗 / 担当者を増やして StoreSelect / StaffSelect が描画される状態にする.
		const { execSync } = require('node:child_process');
		const path = require('node:path');
		execSync(`npx wp-env run cli wp db query "INSERT INTO wp_smb_stores (name, phone, email, prefecture, city, address_line, description, image_id, calendar_color, is_active, sort_order, created_at, updated_at) VALUES ('店舗B', '', '', '', '', '', '', 0, '#10b981', 1, 20, NOW(), NOW());"`, {
		...
		expect(scrollWidth, '横スクロールが発生していない').toBeLessThanOrEqual(clientWidth + 1);

		// 3. カードリストが grid-template-columns: 1fr（1 列）であること.
		const cards = page.locator('.smb-front-cards');
		const gridCols = await cards.evaluate((el) => getComputedStyle(el).gridTemplateColumns);
		// 375px では 1 列のはず（"NUMpx" 単一値）.
		expect(gridCols.split(' ').length, `grid-template-columns: ${gridCols}`).toBe(1);

		// 4. 各カードのタップ領域が 44px 以上.
		const cardCount = await page.locator('.smb-front-card').count();
		expect(cardCount).toBeGreaterThanOrEqual(2);
		for (let i = 0; i < cardCount; i++) {
			const box = await page.locator('.smb-front-card').nth(i).boundingBox();
			expect(box.height, `card[${i}] height`).toBeGreaterThanOrEqual(44);
		}
	});

	test('375px: 日タイルが横スクロール可能で、タイルが 44px 以上のタップ領域を持つ', async ({ page }) => {
		await page.setViewportSize({ width: 375, height: 667 });
		...
		expect(overflowX, `overflow-x: ${overflowX}`).toMatch(/auto|scroll/);
		...
		expect(firstBox.height, 'day-tile height').toBeGreaterThanOrEqual(44);
		...
		expect(scrollInfo.scroll, 'strip scrollWidth > clientWidth').toBeGreaterThan(scrollInfo.client);
	});

	test('375px: 月グリッドが 7列で、セルが 32px 以上の最小幅を持つ', async ({ page }) => {
	...
	test('375px: 時間枠ボタンが 2列以上で各 44px 以上の高さ', async ({ page }) => {
		await page.setViewportSize({ width: 375, height: 667 });
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		...
		const gridCols = await list.evaluate((el) => getComputedStyle(el).gridTemplateColumns);
		const colCount = gridCols.split(' ').length;
		expect(colCount, `grid cols: ${gridCols}`).toBeGreaterThanOrEqual(2);
	...
	test('375px: フォーム input は font-size 16px（iOS ズーム防止）+ width 100%', async ({ page }) => {
		await page.setViewportSize({ width: 375, height: 667 });
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		...
		for (const key of inputs) {
			...
			expect(fontSize, `${key} font-size >= 16px`).toBeGreaterThanOrEqual(16);
	...
	test('375px: 確認画面のラベル/値が縦スタック表示で横スクロールしない', async ({ page }) => {
		await page.setViewportSize({ width: 375, height: 667 });
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
	...
	test('375px: 完了画面が中央寄せで予約番号が見やすく表示される', async ({ page }) => {
		await page.setViewportSize({ width: 375, height: 667 });
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		...
		await expect(page.getByRole('heading', { name: 'ご予約ありがとうございました' })).toBeVisible({ timeout: 10_000 });

		// .smb-front-done が text-align: center.
		const done = page.locator('.smb-front-done');
		const textAlign = await done.evaluate((el) => getComputedStyle(el).textAlign);
	...
	test('375px: カラーカスタマイズ（デザイン設定）がモバイルでも反映される', async ({ page }) => {
		...
		const colorButton = await page.evaluate(() => {
			...
			return el ? getComputedStyle(el).getPropertyValue('--smb-front-color-button').trim() : '';
		});
		expect(colorButton.toLowerCase()).toContain('#ff0066');

		const colorFocus = await page.evaluate(() => {
			const el = document.getElementById('smart-booking-app');
			return el ? getComputedStyle(el).getPropertyValue('--smb-front-color-focus').trim() : '';
		});
	...
	test('768px: 主要要素がレイアウト崩れなく表示される（カード 2 列、横スクロール無し）', async ({ page }) => {
		...
		const cards = page.locator('.smb-front-cards');
		const gridCols = await cards.evaluate((el) => getComputedStyle(el).gridTemplateColumns);
		expect(gridCols.split(' ').length, `768px grid-template-columns: ${gridCols}`).toBe(2);
	...
	test('768px: フォーム → 確認 → 完了 までレイアウト崩れなく表示される', async ({ page }) => {
		await page.setViewportSize({ width: 768, height: 1024 });
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		...
		const gridCols = await list.evaluate((el) => getComputedStyle(el).gridTemplateColumns);
		expect(gridCols.split(' ').length, `confirm__list cols: ${gridCols}`).toBe(2);
	...
	test('a11y: キーボードのみで日付選択 → 時間枠 → フォーム入力 → 確認画面まで遷移できる', async ({ page }) => {
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		...
		await page.locator('.smb-front-day-tile:not(.is-disabled)').first().focus();
		const focused = await page.evaluate(() => document.activeElement && document.activeElement.className);
		expect(focused, `active class: ${focused}`).toContain('smb-front-day-tile');
		await page.keyboard.press('Enter');

		// 時間枠ボタン. 最初の枠にフォーカス → Enter.
		await expect(page.locator('.smb-front-time-btn:not(:disabled)').first()).toBeVisible();
		await page.locator('.smb-front-time-btn:not(:disabled)').first().focus();
		await page.keyboard.press('Enter');
		...
		// 確認画面.
		await expect(page.getByRole('heading', { name: '予約内容の確認' })).toBeVisible();
		await expect(page.locator('.smb-front-confirm')).toContainText('キーボード 太郎');
	});

	test('a11y: focus-visible でフォーカスリングが視覚的に表示される（outline-width 2px）', async ({ page }) => {
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		...
		for (let i = 0; i < 50; i++) {
			...
			activeIsTile = await page.evaluate(() => {
				...
				return !!a && a.classList && a.classList.contains('smb-front-day-tile');
			});
			if (activeIsTile) break;
		}
		expect(activeIsTile, 'Tab で .smb-front-day-tile にフォーカスが到達する').toBe(true);
		...
		expect(px, `outline-width: ${outlineWidth}`).toBeGreaterThanOrEqual(2);
		...
		expect(outlineStyle, `outline-style: ${outlineStyle}`).not.toBe('none');
	});

	test('a11y: 店舗・担当者・日付・時間枠の aria-label が日本語で意味のある形式', async ({ page }) => {
		const { execSync } = require('node:child_process');
		const path = require('node:path');
		execSync(`npx wp-env run cli wp db query "INSERT INTO wp_smb_stores (name, phone, email, prefecture, city, address_line, description, image_id, calendar_color, is_active, sort_order, created_at, updated_at) VALUES ('銀座店', '', '', '', '', '', '', 0, '#10b981', 1, 20, NOW(), NOW());"`, {
			cwd: path.resolve(__dirname, '..', '..'),
			encoding: 'utf8',
			stdio: ['ignore', 'pipe', 'pipe'],
		});
		execSync(`npx wp-env run cli wp db query "INSERT INTO wp_smb_staff (store_id, name, email, phone, description, image_id, sort_order, is_active, created_at, updated_at) VALUES (1, '佐藤 美咲', '', '', '', 0, 20, 1, NOW(), NOW());"`, {
			cwd: path.resolve(__dirname, '..', '..'),
			encoding: 'utf8',
			stdio: ['ignore', 'pipe', 'pipe'],
		});
	...
	test('a11y: フォームエラーが role="alert" で読み上げられる', async ({ page }) => {
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		...
		const invalidInputs = await page.locator('input[aria-invalid="true"], textarea[aria-invalid="true"]').count();
		expect(invalidInputs, 'aria-invalid=true on at least one field').toBeGreaterThan(0);
	});

	test('a11y: 必須フィールドに aria-required="true" が付与されている', async ({ page }) => {
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		...
		const requiredMarks = page.locator('.smb-front-form__required[aria-label="必須"]');
		expect(await requiredMarks.count()).toBeGreaterThanOrEqual(3);
	});

	test('a11y: ハニーポットフィールドが aria-hidden + tabindex=-1 でアクセス不可', async ({ page }) => {
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		...
		const left = await input.evaluate((el) => getComputedStyle(el.parentElement.parentElement).left);
		// .smb-front-honeypot の left を見る. 上の getComputedStyle は input の親（label）→ .smb-front-honeypot 親.
		const honeypotLeft = await wrap.evaluate((el) => getComputedStyle(el).left);
		expect(honeypotLeft, `honeypot left: ${honeypotLeft}`).toBe('-9999px');
	});

	test('a11y: 見出し階層 h2/h3 が論理的（StepHeader=h2, 確認画面のグループタイトル=h3）', async ({ page }) => {
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);

		// 日付選択ステップ.
		await expect(page.locator('h2.smb-front-step-header__title')).toBeVisible();

		await page.waitForSelector('.smb-front-day-tile:not(.is-disabled)', { timeout: 10_000 });
		await page.locator('.smb-front-day-tile:not(.is-disabled)').first().click();
		await page.getByRole('button', { name: /10:00から11:00/ }).click();
		await expect(page.getByRole('heading', { level: 2, name: 'お客様情報の入力' })).toBeVisible();
		...
		await expect(page.getByRole('heading', { level: 2, name: '予約内容の確認' })).toBeVisible();
		await expect(page.getByRole('heading', { level: 3, name: 'ご予約内容' })).toBeVisible();
		await expect(page.getByRole('heading', { level: 3, name: 'お客様情報' })).toBeVisible();

		// 完了画面: h2=「ご予約ありがとうございました」.
		await page.getByRole('button', { name: '予約を確定する' }).click();
		await expect(page.getByRole('heading', { level: 2, name: 'ご予約ありがとうございました' })).toBeVisible({ timeout: 10_000 });
	});

	test('a11y: prefers-reduced-motion でアニメーション/トランジションが無効化される', async ({ browser }) => {
		...
		try {
			...
			await page.waitForSelector('.smb-front-day-tile:not(.is-disabled)', { timeout: 10_000 });
			const tile = page.locator('.smb-front-day-tile:not(.is-disabled)').first();
			const transition = await tile.evaluate((el) => getComputedStyle(el).transitionDuration);
			// transition-duration: 0s（none に設定済）の状態を期待.
			expect(transition, `tile transition: ${transition}`).toBe('0s');

			// btn の transition も 0s.
			await tile.click();
			const btn = page.locator('.smb-front-time-btn').first();
			const btnTransition = await btn.evaluate((el) => getComputedStyle(el).transitionDuration);
	...
	test('print: @media print でアクションボタンが非表示', async ({ page }) => {
		seedWeekSchedules(1, 1);
		await gotoFrontForm(page);
		...
		expect(display, `actions display in print: ${display}`).toBe('none');
	});
});
