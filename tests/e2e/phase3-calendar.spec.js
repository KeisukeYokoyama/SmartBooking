...
test.describe('Phase 3 Eval-2: カレンダーUI 詳細', () => {
	...
	test('mode=day_only: 日ストリップだけ表示、月グリッド・トグル無し', async ({ page }) => {
	...
	test('mode=month_only: 月グリッドだけ表示、日ストリップ・トグル無し', async ({ page }) => {
	...
	test('mode=toggle: 日/月を切替できる、初期は日表示', async ({ page }) => {
	...
	test('日表示: display_period_days=7 で 7 タイル表示、横スクロール (overflow-x:auto)', async ({ page }) => {
		setOption('smb_calendar_view_mode', 'day_only');
	...
	test('日表示: display_period_days=10 でタイルが 10 個表示される（月跨ぎでも連続）', async ({ page }) => {
		setOption('smb_calendar_view_mode', 'day_only');
	...
	test('日表示: 曜日・日付・状態バッジが表示される（few_left は「残りわずか」、full は「満席」）', async ({
		...
		const firstWd = await page.locator('.smb-front-day-tile__weekday').first().textContent();
		expect(['日', '月', '火', '水', '木', '金', '土']).toContain((firstWd || '').trim());
		...
		// few_left バッジ.
		await expect(fewTile.locator('.smb-front-day-tile__badge.is-few')).toContainText('残りわずか');

		const fullDayNum = new Date(fullDate).getDate();
		const fullTile = page.locator('.smb-front-day-tile').filter({
			has: page.locator(`.smb-front-day-tile__day:text("${fullDayNum}")`),
		});
		await expect(fullTile.locator('.smb-front-day-tile__badge.is-full')).toContainText('満席');
		// 満席タイルは disabled.
	...
	test('月表示: 月ヘッダ + 7×N グリッド + 月ナビボタン', async ({ page }) => {
		setOption('smb_calendar_view_mode', 'month_only');
		setOption('smb_display_days', 7);
		seedDailySlots(1, 1);
		await gotoFrontForm(page);
		...
		// 月ラベル: 「2026年4月」のような形式.
		const label = await page.locator('.smb-front-month__label').textContent();
		expect(label).toMatch(/\d{4}年\d{1,2}月/);

		// 曜日ヘッダ 7 個.
		await expect(page.locator('.smb-front-month__weekday')).toHaveCount(7);

		// セル数は 7 の倍数 (28 / 35 / 42).
		const cellCount = await page.locator('.smb-front-month__cell').count();
	...
	test('月表示: 範囲外の月ナビは disabled、範囲内なら有効', async ({ page }) => {
		setOption('smb_calendar_view_mode', 'month_only');
		setOption('smb_display_days', 7);
		seedDailySlots(1, 1);
		await gotoFrontForm(page);
	...
	test('月表示: 隣月セル (is-other-month) は disabled でクリック不可', async ({ page }) => {
		setOption('smb_calendar_view_mode', 'month_only');
		seedDailySlots(1, 1);
		await gotoFrontForm(page);
		await page.waitForSelector('.smb-front-month', { timeout: 10_000 });

		const otherMonthCells = page.locator('.smb-front-month__cell.is-other-month');
	...
	test('日付選択後: カレンダー直下に時間枠 region が現れる（次ステップに遷移しない）', async ({ page }) => {
		setOption('smb_calendar_view_mode', 'day_only');
		// 同じ日に 2 枠 (10:00 / 14:00).
		const d = ymd(1);
		insertSchedulesBulk([
			{ storeId: 1, staffId: 1, date: d, start: '10:00:00', end: '11:00:00', capacity: 3 },
			{ storeId: 1, staffId: 1, date: d, start: '14:00:00', end: '15:00:00', capacity: 3 },
		]);
		await gotoFrontForm(page);
		await page.waitForSelector('.smb-front-day-tile:not(.is-disabled)', { timeout: 10_000 });

		// 日付選択前は時間枠 region 非表示.
		await expect(page.locator('.smb-front-time-slots')).toHaveCount(0);

		await page.locator('.smb-front-day-tile:not(.is-disabled)').first().click();

		// region が表示される.
		await expect(page.getByRole('region', { name: '選択した日の時間枠' })).toBeVisible();
	...
	test('異なる日付を選び直すと時間枠リストが切り替わる', async ({ page }) => {
		setOption('smb_calendar_view_mode', 'day_only');
		const d1 = ymd(1);
		const d2 = ymd(2);
		insertSchedulesBulk([
			{ storeId: 1, staffId: 1, date: d1, start: '10:00:00', end: '11:00:00', capacity: 3 },
			{ storeId: 1, staffId: 1, date: d2, start: '15:00:00', end: '16:00:00', capacity: 3 },
		]);
		await gotoFrontForm(page);
		await page.waitForSelector('.smb-front-day-tile:not(.is-disabled)', { timeout: 10_000 });
		...
		await expect(page.locator('.smb-front-time-btn').first()).toContainText('10:00');

		// 2 枚目 (d2) を選択 → 15:00 に切り替わる.
		await tiles.nth(1).click();
		await expect(page.locator('.smb-front-time-btn')).toHaveCount(1);
		await expect(page.locator('.smb-front-time-btn').first()).toContainText('15:00');
	});

	test('時間枠ボタンクリックで form ステップへ遷移する', async ({ page }) => {
		setOption('smb_calendar_view_mode', 'day_only');
		seedDailySlots(1, 1);
		await gotoFrontForm(page);
		await page.waitForSelector('.smb-front-day-tile:not(.is-disabled)', { timeout: 10_000 });
	...
	test('時間枠: available/few_left/full/closed の各状態でラベルと disabled が正しい', async ({ page }) => {
		setOption('smb_calendar_view_mode', 'day_only');
		// 4 枠を同じ日に作る (capacity=10 で few_left を出しやすくする).
		const d = ymd(1);
		insertSchedulesBulk([
			// available: capacity=10 / booked=0.
			{ storeId: 1, staffId: 1, date: d, start: '09:00:00', end: '10:00:00', capacity: 10 },
			// few_left: capacity=10 / booked=7 (>= ceil(10*0.7)=7).
			{ storeId: 1, staffId: 1, date: d, start: '10:00:00', end: '11:00:00', capacity: 10 },
			// full: capacity=2 / booked=2.
			{ storeId: 1, staffId: 1, date: d, start: '11:00:00', end: '12:00:00', capacity: 2 },
		...
		await expect(avail.locator('.smb-front-time-btn__badge')).toHaveCount(0);

		// 10:00-11:00 = few_left.
		const few = page.getByRole('button', { name: /^10:00から11:00 残りわずか$/ });
		await expect(few).toBeEnabled();
		await expect(few).toHaveClass(/is-few_left/);
		await expect(few.locator('.smb-front-time-btn__badge')).toContainText('残りわずか');

		// 11:00-12:00 = full.
		const full = page.getByRole('button', { name: /^11:00から12:00 満席 選択不可$/ });
		await expect(full).toBeDisabled();
		await expect(full).toHaveClass(/is-full/);
		await expect(full.locator('.smb-front-time-btn__badge')).toContainText('満席');
	});
	...
	test('deadline_days=3: 4日後の枠は available、2日後の枠は closed (締切超過)', async ({ page }) => {
		setOption('smb_calendar_view_mode', 'day_only');
		setOption('smb_display_days', 7);
		...
		insertSchedulesBulk([
			{ storeId: 1, staffId: 1, date: d4, start: '10:00:00', end: '11:00:00', capacity: 3 },
			{ storeId: 1, staffId: 1, date: d2, start: '10:00:00', end: '11:00:00', capacity: 3 },
		]);
		...
		const tile4 = page.locator('.smb-front-day-tile').filter({
			has: page.locator(`.smb-front-day-tile__day:text("${day4Num}")`),
		});
		...
		const tile2 = page.locator('.smb-front-day-tile').filter({
			has: page.locator(`.smb-front-day-tile__day:text("${day2Num}")`),
		});
		await expect(tile2).toBeDisabled();
		await expect(tile2.locator('.smb-front-day-tile__badge.is-closed')).toContainText('締切');
	});
	...
	test('deadline_hours=2: 当日から1時間後の枠は closed (実時間で過去枠を毎日 02:00 へ作る検証)', async ({
		...
		await expect(todayTile.locator('.smb-front-day-tile__badge.is-closed')).toContainText('締切');
	});
	...
	test('両方設定: deadline_days=5 と deadline_hours=2 → 厳しい方 (5日) が適用され3日後は closed', async ({
		...
		const tile = page.locator('.smb-front-day-tile').filter({
			has: page.locator(`.smb-front-day-tile__day:text("${day3Num}")`),
		});
		// 5日前 (=厳しい方) が適用されて closed になる.
		await expect(tile).toBeDisabled();
		await expect(tile.locator('.smb-front-day-tile__badge.is-closed')).toContainText('締切');
	});
	...
	test('display_period_days=14: 日表示で 14 タイル、月表示で 8日目以降も範囲内', async ({ page }) => {
		setOption('smb_calendar_view_mode', 'day_only');
	...
	test('display_period_days=3: 日表示で 4日目以降は表示されない', async ({ page }) => {
		setOption('smb_calendar_view_mode', 'day_only');
	...
	test('color_date_selected: CSS カスタムプロパティが root に適用される', async ({ page }) => {
		...
		const propVal = await page
			...
			.evaluate((el) => el.style.getPropertyValue('--smb-front-color-date-selected'));
		expect(propVal.trim().toLowerCase()).toBe('#ff00ff');
	});

	test('color_time_selected: CSS カスタムプロパティが root に適用される', async ({ page }) => {
		...
		const propVal = await page
			.locator('#smart-booking-app')
			.evaluate((el) => el.style.getPropertyValue('--smb-front-color-time-selected'));
