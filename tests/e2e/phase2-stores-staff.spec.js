...
test.describe( 'Phase 2: 店舗・担当者 管理', () => {
	...
	test( '店舗タブにデフォルトの店舗が表示される', async ( { page } ) => {
		...
		// タブ.
		await expect(
			page.locator( '.smb-tab', { hasText: '店舗' } )
		).toBeVisible();
		await expect(
			page.locator( '.smb-tab', { hasText: '担当者' } )
		).toBeVisible();
		// デフォルト店舗のカードが 1 枚.
		await expect(
	...
	test( '店舗を追加できる（モーダル → 入力 → 追加 → カードに反映）', async ( {
		page,
	} ) => {
		...
		const before = countTable( 'wp_smart_booking_stores' );
		await page
			.getByRole( 'button', { name: /店舗を追加/ } )
			.first()
			.click();
		await expect(
			page.locator( '.smb-modal__title', { hasText: '店舗を追加' } )
		).toBeVisible();
		await page
		...
		const after = countTable( 'wp_smart_booking_stores' );
	...
	test( '必須項目（店舗名）未入力のとき追加ボタンを押すとバリデーションエラーが表示される', async ( {
		page,
	} ) => {
		await page
			.getByRole( 'button', { name: /店舗を追加/ } )
		...
		// エラー文言が表示される.
		await expect(
			page.locator( '.smb-field__error', { hasText: '店舗名は必須' } )
		).toBeVisible();
		// モーダルはまだ開いている.
		await expect(
			page.locator( '.smb-modal__title', { hasText: '店舗を追加' } )
	...
	test( '無効な HEX カラーコードで保存するとバリデーションエラーが表示される', async ( {
		page,
	} ) => {
		await page
			.getByRole( 'button', { name: /店舗を追加/ } )
		...
		await page
			.locator( 'input[aria-label="カラーコード"]' )
			.fill( 'not-a-color' );
		await page
			.locator( '.smb-modal__footer' )
			.getByRole( 'button', { name: /追加する/ } )
			.click();
		await expect(
			page.locator( '.smb-field__error', { hasText: /カラーコード/ } )
		).toBeVisible();
	} );
	...
	test( '無効なメールアドレスでバリデーションエラーが表示される', async ( {
		...
		await page
			...
			.click();
		await expect(
			page.locator( '.smb-field__error', { hasText: /メールアドレス/ } )
		).toBeVisible();
	} );
	...
	test( '店舗を編集できる', async ( { page } ) => {
		...
		await page
			...
			.click();
		await expect(
			page.locator( '.smb-modal__title', { hasText: '店舗を編集' } )
		).toBeVisible();
		...
		await expect(
			page.locator( '.smb-card__title', { hasText: '編集後の名前' } )
		).toBeVisible();
	} );
	...
	test( '店舗の並び替え（↑↓）が動作する', async ( { page } ) => {
		...
		const secondTitle = (
			await secondCard.locator( '.smb-card__title' ).textContent()
		)?.trim();
	...
	test( '予約が紐づいている店舗は削除できず警告が表示される（409）', async ( {
		...
		await page
			...
			.click();
		await expect(
			page.locator( '.smb-modal__title', { hasText: '店舗を削除' } )
		).toBeVisible();
		await page.getByRole( 'button', { name: '削除する' } ).click();
		// エラートーストに「予約」の文字.
		await expect(
			page.locator( '.smb-toast--error' ).last()
		).toContainText( /予約/, { timeout: 6000 } );
		// カードは残っている.
		await expect(
			page.locator( '.smb-card-list article.smb-card' )
		).toHaveCount( 1 );
	...
	test( '担当者タブ: 担当者の追加・編集・削除', async ( { page } ) => {
		...
		// 追加.
		await page
			.getByRole( 'button', { name: /担当者を追加/ } )
			.first()
			.click();
		await expect(
			page.locator( '.smb-modal__title', { hasText: '担当者を追加' } )
		).toBeVisible();
		await page
		...
		await expect(
			page.locator( '.smb-card__title', { hasText: 'テスト花子' } )
		).toBeVisible();

		// 編集.
		await page
			.locator( '.smb-card', { hasText: 'テスト花子' } )
			.getByRole( 'button', { name: '編集' } )
			.click();
		...
		// 削除.
		await page
			.locator( '.smb-card', { hasText: 'テスト花子_改' } )
			.getByRole( 'button', { name: '削除' } )
			.click();
		await page.getByRole( 'button', { name: '削除する' } ).click();
		await expect(
	...
	test( '担当者必須項目（名前）のバリデーション', async ( { page } ) => {
		...
		await page
			...
			.click();
		await expect(
			page.locator( '.smb-field__error', { hasText: /担当者名は必須/ } )
		).toBeVisible();
	} );

	test( '予約が紐づいている担当者は削除できず警告が表示される（409）', async ( {
		page,
	} ) => {
