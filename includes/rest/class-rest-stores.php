...
class Smart_Booking_REST_Stores extends Smart_Booking_REST_Base {
	...
	public function create_item( $request ) {
		...
		if ( (int) $data['sort_order'] <= 0 ) {
			...
			$max_order          = (int) $wpdb->get_var( "SELECT MAX(sort_order) FROM {$wpdb->prefix}smart_booking_stores WHERE is_system = 0" );
	...
	public function reorder( $request ) {
		...
		if ( ! is_array( $items ) ) {
			return $this->error( 'smb_store_reorder_invalid', '並び替えデータの形式が正しくありません。', 400 );
		}
