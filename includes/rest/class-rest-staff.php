 ...
 *
 * smart_booking_staff テーブルに対する CRUD。
...
class Smart_Booking_REST_Staff extends Smart_Booking_REST_Base {
	...
	private function table() {
		global $wpdb;
		return $wpdb->prefix . 'smart_booking_staff';
	}
	...
	public function get_items( $request ) {
		...
		$sql = "SELECT * FROM {$wpdb->prefix}smart_booking_staff WHERE " . implode( ' AND ', $where ) . ' ORDER BY sort_order ASC, id ASC';
		if ( ! empty( $params ) ) {
	...
	public function get_item( $request ) {
		...
		$row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}smart_booking_staff WHERE id = %d", $id ), ARRAY_A );
		if ( ! $row ) {
	...
	private function sanitize_input( $request ) {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$store_exists = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}smart_booking_stores WHERE id = %d", $store_id ) );
		if ( 0 === $store_exists ) {
	...
	public function update_item( $request ) {
		...
		$id = (int) $request['id'];
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$exists = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}smart_booking_staff WHERE id = %d", $id ) );
		if ( 0 === $exists ) {
			return $this->error( 'smb_staff_not_found', '指定された担当者が見つかりません。', 404 );
		}
	...
	public function delete_item( $request ) {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$row = $wpdb->get_row( $wpdb->prepare( "SELECT id, is_system FROM {$wpdb->prefix}smart_booking_staff WHERE id = %d", $id ), ARRAY_A );
		if ( ! $row ) {
			return $this->error( 'smb_staff_not_found', '指定された担当者が見つかりません。', 404 );
		}
		...
		$reservation_count = (int) $wpdb->get_var(
			$wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}smart_booking_reservations WHERE staff_id = %d", $id )
		);
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$wpdb->delete( $wpdb->prefix . 'smart_booking_schedules', array( 'staff_id' => $id ), array( '%d' ) );

		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$wpdb->delete( $wpdb->prefix . 'smart_booking_staff', array( 'id' => $id ), array( '%d' ) );
