 ...
 *
 * smart_booking_schedules テーブルに対する CRUD + コピー。
...
class Smart_Booking_REST_Schedules extends Smart_Booking_REST_Base {
	...
	private function table() {
		global $wpdb;
		return $wpdb->prefix . 'smart_booking_schedules';
	}
	...
	public function get_items( $request ) {
		...
		$sql = "SELECT * FROM {$wpdb->prefix}smart_booking_schedules WHERE " . implode( ' AND ', $where ) . ' ORDER BY schedule_date ASC, start_time ASC';
		if ( ! empty( $params ) ) {
	...
	public function get_item( $request ) {
		...
		$row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}smart_booking_schedules WHERE id = %d", $id ), ARRAY_A );
		if ( ! $row ) {
	...
	private function get_system_store_id() {
		global $wpdb;
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$id = (int) $wpdb->get_var( "SELECT id FROM {$wpdb->prefix}smart_booking_stores WHERE is_system = 1 ORDER BY id ASC LIMIT 1" );
		return $id;
	}
	...
	private function get_system_staff_id( $store_id = 0 ) {
		...
		if ( $store_id > 0 ) {
			...
			$id = (int) $wpdb->get_var(
				$wpdb->prepare(
					"SELECT id FROM {$wpdb->prefix}smart_booking_staff WHERE is_system = 1 AND store_id = %d ORDER BY id ASC LIMIT 1",
					$store_id
				)
			);
			if ( $id > 0 ) {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		return (int) $wpdb->get_var( "SELECT id FROM {$wpdb->prefix}smart_booking_staff WHERE is_system = 1 ORDER BY id ASC LIMIT 1" );
	}
	...
	public function create_item( $request ) {
		...
		foreach ( $batch as $one ) {
			...
			$existing = $wpdb->get_row(
				$wpdb->prepare(
					"SELECT id, booked_count FROM {$wpdb->prefix}smart_booking_schedules WHERE store_id = %d AND staff_id = %d AND schedule_date = %s AND start_time = %s LIMIT 1",
					$data['store_id'],
					$data['staff_id'],
					$data['schedule_date'],
					$data['start_time']
	...
	public function update_item( $request ) {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$wpdb->update( $wpdb->prefix . 'smart_booking_schedules', $update, array( 'id' => $id ), $format, array( '%d' ) );
	...
	public function delete_item( $request ) {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$exists = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}smart_booking_schedules WHERE id = %d", $id ) );
		if ( 0 === $exists ) {
			return $this->error( 'smb_schedule_not_found', '指定されたスケジュールが見つかりません。', 404 );
		}

		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$used = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}smart_booking_reservations WHERE schedule_id = %d", $id ) );
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$wpdb->delete( $wpdb->prefix . 'smart_booking_schedules', array( 'id' => $id ), array( '%d' ) );
		return rest_ensure_response(
			array(
				'deleted' => true,
				'id'      => $id,
	...
	public function copy_schedules( $request ) {
		...
		$sql = "SELECT * FROM {$wpdb->prefix}smart_booking_schedules WHERE " . implode( ' AND ', $where );
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter
